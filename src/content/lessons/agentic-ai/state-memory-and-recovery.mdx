...
<Callout kind="breaks" title="Memory is context, not authority">
A remembered instruction — however confidently stored — doesn't outrank current permissions or policy. Memory supplies context; authorization stays an application-layer decision evaluated fresh.
</Callout>
...
<Compare>
<Pane title="Orchestration view" tone="neutral">
State makes a run restartable and observable — every step resumable or safely repeatable.
</Pane>
<Pane title="Privacy view" tone="good">
Memory is retained user data: it needs a purpose, an owner, a scope, and a deletion path — not just a place to live.
</Pane>
</Compare>
...
**Related:** [Agent memory, state, and world models](/learn/classical-ai/agents-robotics/305-agent-memory-state-and-world-models)
