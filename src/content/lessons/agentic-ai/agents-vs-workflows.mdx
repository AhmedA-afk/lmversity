...
<Callout kind="breaks" title="Autonomy on a deterministic task">
An agent on a three-step form buys nondeterminism, cost, and a new failure surface — in exchange for nothing code couldn't express. Autonomy must earn its place against a known sequence.
</Callout>
...
<Compare>
<Pane title="Capability view" tone="good">
An agent is useful when deciding the next bounded action is the hard part.
</Pane>
<Pane title="Reliability view" tone="neutral">
Every choice point is a new failure surface — autonomy must earn its place with tests and recovery.
</Pane>
</Compare>
...
- Continue: [State, memory, and recovery](/learn/agentic-ai/state-memory-and-recovery)
