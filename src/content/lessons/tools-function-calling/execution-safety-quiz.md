...
B) `read`, safe to auto-run, but still needs an authorization check that `customer_id` belongs to the caller
C) `irreversible`, because financial data is sensitive
D) No tier needed — read-only tools skip the registry entirely

<details>
<summary>Answer</summary>

**Correct: B.** Tiering is about side effects, and this tool has none — `read` is right. But "safe to auto-run" describes whether it needs a *gate*, not whether it needs *authorization*; a read-only tool that skips the ownership check is exactly the [Never Trust the Model's Arguments](/learn/tools-function-calling/validating-tool-arguments) cross-tenant-read failure. (A) confuses "touches the database" with "has a side effect" — reads touch the database too. (C) over-classifies by subject matter rather than by reversibility — sensitivity affects what you log and how carefully you authorize, not whether the read itself needs a human gate. (D) is wrong on principle: every tool goes through the registry, including reads, so the dispatcher can validate and log consistently.
...
B) The function name should be `fetch_document`, not `get_document`
C) The handler runs with the service's own broad database credentials and never checks that `ctx.user_id` (or an equivalent authenticated identity) actually has rights to this specific `doc_id`
D) There's no risk — this is a read-only tool

<details>
<summary>Answer</summary>

**Correct: C.** This is the exact shape from [The Confused-Deputy Problem](/learn/tools-function-calling/the-authority-problem): the agent's credential (broad, can read any document) is exercised with no check that this caller's authority (narrow, should read only their own) actually covers this call. Any `doc_id` the model produces — hallucinated, guessed, or lifted from injected content — gets served. (A) is a real gap too (shape validation belongs in the args model), but it isn't the *authority* problem the question asks about — a perfectly well-typed `doc_id` still leaks data without an ownership check. (B) is irrelevant naming trivia. (D) repeats the mistake in the code itself — "read-only" bounds the effect of a single call, not who it's scoped to; a read that returns the wrong person's data is still a real breach.
...
A) Treat it as untrusted data, not an instruction — delimit fetched content explicitly in the prompt, and gate any consequential call that follows a fetch in the same turn
B) The model should comply — it's a reasonable-sounding instruction and the tool call itself is harmless
...
B) Define a domain-appropriate threshold — e.g. gate only if it invites external attendees or is on a shared/public calendar, and auto-run a private single-user event
C) Always gate it, since a threshold-based policy doesn't apply cleanly
D) Never gate it — it's reversible, so no gate is ever needed

<details>
<summary>Answer</summary>

**Correct: B.** [Classifying Tools by Risk Tier](/learn/tools-function-calling/classifying-tool-risk-tiers) frames the threshold as being about blast radius, not a specific numeric field — "does this call reach outside the current conversation/user" is the real question, and for a calendar tool that's naturally "does it involve other people or a shared calendar," not a dollar figure. (A) misclassifies a real side effect as a read. (C) over-gates a tool where the low-stakes case (a private reminder on your own calendar) is genuinely fine to auto-run — gating everything trains reviewers to stop reading, the interruption-fatigue problem from [Human-in-the-Loop Approval Gates](/learn/tools-function-calling/approval-gates-design). (D) ignores that "reversible" isn't the only axis — inviting five external clients to the wrong meeting is reversible but still worth a glance before it sends notifications to people outside your organization.
