...
This subset is exactly what [Designing a Tool Schema](/learn/tools-function-calling/designing-a-tool-schema) walks through for a `create_calendar_event` tool, and it's the same subset [JSON Schema: Specifying Your Exact Data Contract](/learn/structured-outputs/json-schema-for-outputs) uses for constraining a final answer rather than a tool call — the two contexts share a schema dialect even though one shapes an argument object and the other shapes a response body. If you're generating schemas mechanically from an existing API, [Converting an OpenAPI Spec into Tool Schemas](/learn/tools-function-calling/openapi-to-tool-schema) shows where OpenAPI's richer vocabulary (nullable, oneOf, discriminators) has to be flattened down into this same essential set. And [Tool Schemas as a Structured-Extraction Mechanism](/learn/structured-outputs/tool-function-schemas) covers the schema object's role in the wider function-calling contract, beyond just its fields.
...
- **Nesting for the sake of "correctness."** A schema that mirrors your database's normalized structure is not a favor to the model. Flatten first; see [Parameter Design Patterns](/learn/tools-function-calling/parameter-design-patterns) for the rule of thumb.

## Where next

Once the fields are second nature, the real skill is choosing what to require and how to word it — that's [Designing a Schema From a Fuzzy Requirement](/learn/tools-function-calling/designing-a-tool-schema-walkthrough), followed by [Writing Descriptions Models Actually Follow](/learn/tools-function-calling/writing-descriptions-models-follow-deep) for the description itself.

**Related:** [Designing a Tool Schema](/learn/tools-function-calling/designing-a-tool-schema) · [The JSON Schema Subset That Matters — outputs angle](/learn/structured-outputs/json-schema-essentials-for-outputs) · [JSON Schema: Specifying Your Exact Data Contract](/learn/structured-outputs/json-schema-for-outputs) · [Tool Schemas as a Structured-Extraction Mechanism](/learn/structured-outputs/tool-function-schemas) · [Converting an OpenAPI Spec into Tool Schemas](/learn/tools-function-calling/openapi-to-tool-schema) · [Parameter Design Patterns](/learn/tools-function-calling/parameter-design-patterns)
