...
B) Escalate to a human for authorization
C) Return the error to the model as a tool result and let it retry
D) Silently retry once, then give up without telling anyone

<details>
<summary>Answer</summary>

**Correct: B.** No amount of retrying or model reasoning grants a permission the service account doesn't have — this is squarely a human-fixable failure, per the routing fork in [Handling Tool Errors and Retries](/learn/tools-function-calling/handling-errors-and-retries).
...
- C is wrong: the model can't fix a permission it has no power to grant itself — sending it back just adds a wasted turn before the failure inevitably needs a human anyway.
- B is correct: only a person can grant the missing permission or explicitly authorize the deletion — this is exactly the escalate-to-user case in [Fail to the Model, the User, or Silently Retry](/learn/tools-function-calling/error-surface-strategies).
...
- C is correct: field, constraint, and value are all present, and nothing internal leaked through.
- B is wrong: this is the raw traceback with different formatting — it still contains an internal file path and a Python-specific exception name that mean nothing to the model's reasoning about the *booking* schema, and it's exactly the leak pattern flagged in [Reliability Mistakes](/learn/tools-function-calling/reliability-common-mistakes).
...
B) A system prompt reminding the model to be efficient
C) Logging every tool call to a file
D) A hard cap on total tool calls per turn

<details>
<summary>Answer</summary>

**Correct: D.** Nothing in this loop can terminate a run where the model keeps generating tool calls indefinitely — a max-call cap is the one guard that bounds the loop unconditionally, regardless of what's causing it to keep going, per [Stopping Runaway Loops](/learn/tools-function-calling/infinite-loop-and-retry-caps).
...
- D is correct: this is the one guard with no failure mode that lets the loop run past it — every other guard (identical-call detection, failure streaks) can be defeated by a loop varying its behavior enough, but a hard count cap always fires.
- B is wrong: a prompt instruction is a soft nudge, not an enforced bound — it can reduce how often the model over-calls, but it provides no guarantee, unlike code that actually counts and stops.
- C is wrong: logging helps you debug a stuck loop after the fact (see [Debugging a Stuck Agent](/learn/tools-function-calling/debugging-a-stuck-agent-loop)) but does nothing to stop one while it's happening.
...
A) Transient — retry after the indicated delay, with backoff if it recurs
B) Deterministic — return it to the model to pick a different tool
...
**Correct: A.** `429` is the textbook transient failure — rate limiting is about *when* you asked, not what you asked, and the API is explicitly telling you how long to wait, per the transient set in [Retry, Back Off, or Give Up](/learn/tools-function-calling/retry-strategies-for-tools).

- B is wrong: nothing about the request itself is invalid — a different tool wouldn't help, and the model has no power over the rate limit anyway.
- A is correct: respect the `Retry-After` hint (or fall back to exponential backoff with jitter if none is given) and retry entirely in code — the model never needs to see this.
