...
<Callout kind="note" title="The refusal path is the graded requirement">
Weekend demos nail the happy path. What separates a capstone from a chatbot demo is behavior when the answer *isn't* in the corpus — hedge or refuse, never confabulate. Build for that on purpose.
</Callout>
...
<InlineCheck
  question="Why does the spec require changing one thing and measuring the eval move — rather than adding several improvements at once?"
  options={['Without a single isolated change, a moved score can\'t be attributed — you\'d never know which lever helped. And a flat score is a real result: report it', 'Because two changes are too slow to build', 'Because evals only test one thing at a time']}
  answer={0}
  explanation="This is what turns 'I think it got better' into 'it got better.' One variable + a rerun golden set gives you a comparable number — the habit that separates engineering from tinkering."
/>
...
1. Show a question where your bot answered from the wrong retrieved chunk. Why did retrieval rank it, and what did you change?
2. Which question type does your system answer worst — and is that a retrieval, chunking, or generation failure? Prove which.
3. Point at the citation your system gave for one answer and check it line by line. Where does the text overclaim the source?
4. If a doc in the corpus contradicts itself across versions, what does your system do? Show the record.
5. What's the difference in your logs between 'retrieval found nothing' and 'retrieval found junk'? Which metric sees that difference?
