...
<Callout kind="breaks" title="Citation laundering">
Citing a relevant page next to a claim that page doesn't support is worse than no citation — it borrows the page's credibility for an unsupported claim. Only claim-level entailment or a span check catches it.
</Callout>
...
<Compare>
<Pane title="Context view" tone="neutral">
Budget tokens like an editor budgets a briefing: relevance, diversity, freshness, and exceptions matter.
</Pane>
<Pane title="Trust view" tone="good">
The question is not "did the model cite something?" but "can a reviewer follow the claim back to an authorized source?"
</Pane>
</Compare>
