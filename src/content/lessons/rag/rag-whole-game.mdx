...
<InlineCheck
  question="Why does the funnel run cheap-and-wide first, then expensive-and-narrow — never the reverse?"
  options={['Cross-encoder reranking reads query+chunk together and is far slower than vector search — run it first and you pay its price on hundreds of chunks you\'d discard anyway. Wide retrieval is cheap; reranking spends it only on survivors', 'Because rerankers can only see 5 chunks', 'Because vector search is more accurate']}
  answer={0}
  explanation="Each stage's cost is justified by what the next stage needs. Retrieve 50 candidates with fast approximate search, then let the expensive accurate model decide which 5 deserved it."
/>
...
**Related:** [What Is RAG and When to Use It](/learn/rag/what-is-rag-and-when-to-use-it) · [Building a RAG Pipeline End to End](/learn/rag/building-a-rag-pipeline-end-to-end) · [Evaluating RAG Quality](/learn/rag/evaluating-rag-quality) · [When RAG Is the Wrong Tool](/learn/rag/when-rag-is-the-wrong-tool) · [RAG Capstone: Support Bot](/learn/rag/rag-capstone-support-bot); [The Whole Game: One Token, End to End](/learn/llm-foundations/whole-game-one-token-end-to-end)
