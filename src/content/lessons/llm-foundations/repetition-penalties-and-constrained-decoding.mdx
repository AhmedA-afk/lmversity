...
Both techniques in this lesson operate on the same object — the logit vector, before softmax — but they make fundamentally different kinds of promises.

<Compare title="A penalty vs. a constraint">
  <Pane title="Repetition penalty" tone="neutral">Makes a repeated token *less likely*. Only ever shifts odds — it never sets a probability to exactly zero.</Pane>
  <Pane title="Constrained decoding" tone="neutral">Makes an invalid token *impossible*. Sets a masked token's logit to `-infinity`, so its probability is exactly zero after softmax.</Pane>
</Compare>

Confusing the two is where most JSON-mode disappointments come from.
...
<InlineCheck
  question="A token has appeared once and a different token has appeared five times. Under a frequency penalty, how do their penalties compare?"
  options={['They receive exactly the same penalty, since both tokens have appeared at least once', 'The token used five times accumulates five times the penalty of the one used once', 'The token used once receives the larger penalty, to discourage any repetition at all']}
  answer={1}
  explanation="Frequency penalty scales with count: logit[token] -= frequency_penalty * count(token). A token seen five times accumulates five times the penalty of one seen once, unlike presence penalty, which is a flat, binary subtraction."
/>

<Callout kind="breaks" title="A penalty can suppress a token that is linguistically mandatory">
Push any of these penalties hard enough and you can suppress a token that's linguistically mandatory — an article, a common preposition, a variable name that legitimately needs to reappear. The penalty has no concept of *why* a token repeated; it only counts occurrences. This is the mechanism behind [sampling parameter mistakes](/learn/llm-foundations/sampling-parameter-mistakes)' warning about applying repetition penalties uniformly across code and numeric output — a variable name or a repeated digit gets the exact same downward pressure as a genuinely stuck phrase, because the penalty can't tell the difference.
</Callout>
...
<InlineCheck
  question="A high temperature is set during sampling. Can it revive a token whose logit was masked to -infinity by constrained decoding?"
  options={['Yes, a high enough temperature can always recover some probability for a masked token', 'No — temperature, top-k, and top-p only redistribute or filter probability mass that already exists, and a masked token has none to redistribute', 'Only if top-p is also set to 1.0']}
  answer={1}
  explanation="A mask sets a token's probability to exactly zero, not merely low. Temperature, top-k, and top-p operate on probability mass that already exists — they have nothing to work with once a candidate is reduced to true zero."
/>
