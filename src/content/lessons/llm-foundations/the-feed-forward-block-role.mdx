...
Give attention and the FFN two different verbs:

<Compare title="Two verbs in one layer">
  <Pane title="Attention">Attention <strong>moves</strong> information between positions.</Pane>
  <Pane title="Feed-forward">The FFN <strong>processes</strong> information at each position.</Pane>
</Compare>

A transformer layer runs one of each, in sequence:
...
<InlineCheck
  question="Which sublayer can make token i's output depend on token j's input, for i ≠ j?"
  options={['The feed-forward network', 'Attention', 'Both, equally']}
  answer={1}
  explanation="Attention is the only place in the architecture where one position's output can depend on another position's input. The feed-forward network runs on each position independently, with no cross-token access."
/>
...
<InlineCheck
  question="Why can't attention alone do the FFN's job of applying a genuinely nonlinear transformation?"
  options={['Attention has no learnable weights', 'Softmax-weighted averaging is a linear combination of existing value vectors, so attention alone can only ever produce blends of what is already present', 'Attention only ever looks at the single most recent token']}
  answer={1}
  explanation="Attention's output is a softmax-weighted average of value vectors — a linear combination of what already exists. It can blend information but cannot, on its own, produce a genuinely new nonlinear function of it. That is exactly the gap the FFN's nonlinearity fills."
/>
...
<Callout kind="breaks" title="The FFN cannot see other tokens">
...
<Callout kind="breaks" title="The FFN is not a minor add-on">
...
<Callout kind="breaks" title="The order of the two sublayers is not a style choice">
