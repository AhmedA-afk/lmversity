...
<Callout kind="breaks" title="Unmasked prompt tokens teach the wrong task">
If prompt tokens weren't masked, the model would spend part of every gradient step learning to predict user text — a task it never performs at inference, and one that could subtly bias generation toward "sounding like a user" in some contexts.
</Callout>

<InlineCheck
  question="Why does SFT mask the loss on the prompt and user tokens instead of computing loss over the whole sequence?"
  options={['Because the model never has to generate the user’s words at inference, so training it to predict them would spend gradient on a task it never performs and could bias it toward sounding like a user', 'Because the prompt tokens are too short to contribute meaningful gradient', 'Because masking reduces the size of the chat template needed']}
  answer={0}
  explanation="At inference the user's turn is given as context, not generated. Masking the loss on those tokens keeps every bit of gradient directed at the assistant response, which is the only thing the model actually has to produce."
/>
...
<Callout kind="breaks" title="Loss-masking bugs are silent">
If the mask boundary is off by one token, the model either loses a trivial amount of signal (usually harmless) or picks up gradient on user text (subtly degrading behavior in ways that are hard to catch without careful evals).
</Callout>
...
2. A mismatch between the template used at training time and the one used at inference time — even a subtly different tokenizer or template version with shifted role-token IDs — silently breaks a fine-tuned model. The template at inference must exactly match what was used during training.
