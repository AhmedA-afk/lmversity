...
<Callout kind="breaks" title="Repetition loops with no way out">
Once greedy locks onto a phrase, the highest-probability continuation of a repeated phrase is often the same phrase again — nothing in the algorithm has a mechanism to notice or escape a loop, because each step only ever looks at the immediate next-token distribution, never the history of its own choices. See [sampling parameter mistakes](/learn/llm-foundations/sampling-parameter-mistakes) for exactly how this shows up in practice.
</Callout>

**Relative cost.** Cheapest possible — one `argmax` per step, no extra bookkeeping.

<InlineCheck
  question="Why can't greedy decoding notice it's stuck in a repetition loop and break out of it?"
  options={[
    'Each step only looks at the immediate next-token distribution, with no mechanism to consider the history of its own previous choices',
    'Greedy decoding applies a very low temperature, which makes loops more likely by design',
    'Greedy decoding filters out any token that already appeared earlier in the sequence'
  ]}
  answer={0}
  explanation="Greedy decoding is argmax applied fresh at every step. It has no memory of the sequence it has already produced beyond what's in the context, so if the highest-probability next token happens to repeat a phrase, nothing stops it from repeating again."
/>
...
<Callout kind="breaks" title="Optimizing for one sequence produces bland text">
Beam search optimizes for the single most probable *sequence*, which in open-ended generation tends to be bland, generic, repetitive text — the token-by-token equivalent of picking the safest word every time, compounded across many tokens. It also doesn't fix greedy's looping tendency; it just does it with `B` candidates instead of one, and it's markedly more expensive for text where there's no single "correct" continuation to converge toward. This is the direct reason open-ended chat rarely uses beam search: a chatbot answer isn't a translation with one right target, it's one plausible answer among many, and beam search's whole design is built around chasing a single best sequence that doesn't really exist for that kind of task.
</Callout>
...
<Callout kind="vendors" title="Support varies by inference stack">
Min-p is newer and less universally supported across inference APIs than top-p, and the right `min_p` value isn't as widely established or intuitive as "0.9 or 0.95" is for top-p — you're more likely to be tuning it from scratch for your use case.
</Callout>
...
<InlineCheck
  question="Why does min-p tend to hold up better than top-p at high temperature?"
  options={[
    'Its threshold scales with the top token’s own probability, so it stays proportionate to the model’s confidence even as temperature flattens the distribution',
    'Min-p disables temperature entirely, so raising it has no effect on the candidate set',
    'Min-p always keeps exactly the same number of tokens regardless of the distribution’s shape'
  ]}
  answer={0}
  explanation="Top-p uses a fixed cumulative-probability threshold, which a flattened high-temperature distribution can blow past with a large, noisy candidate set. Min-p's floor is relative to the top token's own probability, so it shrinks or grows with the model's actual confidence instead."
/>
...
<Steps>
  <Step title="Do you need the exact same output every time?">
    Greedy. Nothing else is deterministic.
  </Step>
  <Step title="Is there one demonstrably correct target sequence?">
    Beam search is worth its cost here — but recognize that most LLM use cases (chat, drafting, summarizing) don't actually have this property, even though it feels intuitive that "the best answer" should exist.
  </Step>
  <Step title="Open-ended generation, no established reason to deviate?">
    Top-p (nucleus) paired with a moderate temperature is the standard default for good reason — it's well-understood, well-supported everywhere, and works.
  </Step>
  <Step title="Running hot and top-p feels noisy?">
    If temperature is well above 1 and top-p feels noisy, try min-p — it's built for exactly this interaction.
  </Step>
  <Step title="Specifically fighting generic, predictable output?">
    Typical sampling is the one strategy actually designed to filter out "obvious" tokens rather than just "unlikely" ones — worth reaching for once top-p and temperature tuning have plateaued.
  </Step>
</Steps>
