...
<Step title="Prefill: all 3 prompt tokens at once">
...
Prefill computes `K`, `V`, and `Q` for every prompt token in a single batched matrix multiply — all 3 rows at once, because the whole prompt is already known and there's no reason to process it one token at a time. This is the parallel phase: one big matmul over `(3, head_dim)` inputs instead of 3 sequential ones. Nothing about it resembles the decode steps that follow.
</Callout>

</Step>

<Step title="Decode step 1: one new token, one appended row">
...
<InlineCheck
  question="After decode step 1 runs, what happens to q4, the query vector that was just used?"
  options={['It gets appended to the cache alongside k4 and v4, for reuse in later steps', 'It is discarded — only k4 and v4 persist in the cache', 'It replaces the oldest row in the cache to keep the cache size fixed']}
  answer={1}
  explanation="The query is computed fresh, used immediately against the current cache, and discarded. Only keys and values persist — that asymmetry is the entire content of what gets cached."
/>
...
<Compare title="Naive recompute vs cached decode step">
  <Pane title="Naive (wrong)" tone="bad">
...
```
  ...
  <Pane title="Cached (right)" tone="good">
...
<InlineCheck
  question="The naive and cached decode steps can produce numerically identical output. What actually differs between them?"
  options={['Nothing measurable — they are the same algorithm written two ways', 'Only the cost: naive redoes O(t) work at every step (O(n²) total), while cached does O(1) work per step (O(n) total)', 'The cached version trades some accuracy for speed by skipping tokens']}
  answer={1}
  explanation="Keys and values don't change once computed, so re-projecting the whole sequence lands on the same values the cache already holds. The difference is purely computational cost — quadratic total work without a cache, linear with one."
/>
