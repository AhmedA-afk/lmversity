...
<Compare title="Which side of the trade is the bottleneck">
  <Pane title="Parameters (N) held fixed, more data (D)" tone="neutral">
    Loss keeps falling until the model runs out of capacity to absorb what the data teaches it — diminishing returns from a model too small for the corpus.
  </Pane>
  <Pane title="Data (D) held fixed, more parameters (N)" tone="neutral">
    Loss keeps falling until the model has memorized everything learnable from that data and starts fitting noise — diminishing returns from a corpus too small for the model.
  </Pane>
</Compare>
...
<InlineCheck
  question="Why does the Chinchilla ratio matter for a fixed compute budget C?"
  options={['Because C ≈ 6·N·D forces a trade between more parameters and more data, and the Chinchilla ratio is the split of that fixed budget that minimizes loss', 'Because it fixes the irreducible loss floor regardless of N or D', 'Because it removes the need to choose between more parameters and more data at all']}
  answer={0}
  explanation="Training compute is approximately 6·N·D, so a fixed compute budget forces a choice between a bigger model and more data. The Chinchilla ratio (roughly 20 tokens per parameter) is the split of that budget that minimizes loss."
/>
...
<Callout kind="breaks" title="A scaling law predicts loss, not capability">
A lower cross-entropy number reliably means better next-token prediction on average — it does not tell you whether a specific downstream skill (multi-digit arithmetic, following a five-step instruction) has crossed a usable threshold. Two runs with the smoothly-predicted loss values can differ wildly on a single benchmark.
</Callout>

<Callout kind="breaks" title="The fitted constants are regime-specific">
Change the data mix, the architecture family, or the tokenizer, and the exponents shift. A scaling law fit on one setup doesn't transfer exactly to another — it has to be re-fit, not assumed.
</Callout>

<Callout kind="breaks" title="The line eventually bends">
High-quality data is finite, and diminishing returns on data quality (not just quantity) show up as the tail runs out. A scaling law extrapolated far past the data you fit it on is a guess dressed as an equation.
</Callout>

<InlineCheck
  question="If two training runs have identical, smoothly-predicted scaling-law loss values, what does that guarantee about their downstream capabilities?"
  options={['Nothing — a lower or matching loss reliably predicts better average next-token prediction, but not whether any specific downstream skill has crossed a usable threshold', 'That both runs will score identically on every benchmark', 'That the runs used the same data mix and architecture']}
  answer={0}
  explanation="A scaling law predicts the loss average, not any specific capability. Two runs with the same predicted loss can still differ sharply on a single downstream benchmark, since loss is a smooth average and a benchmark score is a thresholded, specific skill."
/>
