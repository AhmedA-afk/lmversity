...
import DecodingPipeline from '../../../components/diagrams/DecodingPipeline.astro';
...
<InlineCheck
  question="Why does temperature scaling have to run before top-k/top-p filtering, not after?"
  options={[
    'Filtering decisions depend on the reshaped distribution, so running them first would compute cumulative mass over the wrong shape',
    'Temperature only affects tokens that survive filtering, so the order makes no real difference',
    'Filtering is undefined until the scores have already been renormalized once'
  ]}
  answer={0}
  explanation="Top-k and top-p decide which tokens survive based on the current shape of the distribution. If filtering ran before temperature scaling, it would be filtering the raw, unscaled logits instead of the distribution the reader actually intends to sample from."
/>
...
<Steps>
  <Step title="Scale by temperature">
    At `T = 1`, nothing changes yet; the raw logits pass through untouched (that's the whole point of `T = 1` as a baseline — see [temperature as flattening](/learn/llm-foundations/temperature-as-flattening) for what changing it does to this exact table).
  </Step>
  <Step title="Softmax to get a distribution">
    Exponentiating and normalizing (`exp(logit) / sum(exp(all logits))`) gives:
  ...
  <Step title="Filter">
    Apply top-p at `p = 0.9`: walk down the sorted list adding probability until the running total clears 0.9. `salt` (0.622) + `pepper` (0.229) = 0.851, still under 0.9; add `sugar` (0.084) to reach 0.935, which clears it. Keep exactly those three tokens, zero out the other five.
  </Step>
  <Step title="Renormalize and sample">
    The kept probabilities (0.622, 0.229, 0.084) sum to 0.935, not 1 — divide each by 0.935 to get 0.665, 0.245, 0.090, which do sum to 1. Now draw: `salt` wins about two-thirds of the time, `pepper` about a quarter, `sugar` about one time in eleven, and nothing else can ever be chosen at this step.
  </Step>
</Steps>

<InlineCheck
  question="After filtering keeps salt (0.622), pepper (0.229), and sugar (0.084) at top-p = 0.9, what happens to sugar's probability once renormalization runs?"
  options={[
    'It rises slightly, to about 0.090, because the excluded 6.5% of mass gets redistributed among the survivors',
    'It stays exactly 0.084, since renormalization only touches salt and pepper',
    'It drops, because renormalization divides by a number larger than 1'
  ]}
  answer={0}
  explanation="The three kept probabilities sum to 0.935, not 1, so dividing each by 0.935 pushes every survivor's probability up proportionally — sugar's 0.084 becomes about 0.090."
/>
...
<Callout kind="breaks" title="A 'probability' from an API isn't the model's raw confidence">
By the time you see it, it's already been through temperature scaling and filtering — it reflects your request's settings as much as the model's own belief.
</Callout>

<Callout kind="breaks" title="Top-k and top-p can compose in ways that hide one of them">
Both filters can apply in sequence (implementation-dependent, but common); if top-k=5 already cut the field down, a generous top-p on top of it does nothing, because there's nothing left to include.
</Callout>

<Callout kind="breaks" title="Renormalization changes relative odds among survivors">
The three tokens above keep their *relative* order and rough proportions, but their absolute probabilities all shift upward once the excluded 6.5% of mass is redistributed among them.
</Callout>
