...
A. `[1.0, 0.0]`
B. `[0.73, 0.27]`
C. It depends on the value vectors, which aren't given
D. `[0.5, 0.5]`

<details>
<summary>Answer</summary>

**Correct: D.** `q · k1 = 1` and `q · k2 = 1` — identical scores, so after scaling and softmax they must come out equal.

- **A is incorrect.** This would require one score to dominate the other; here they're identical before and after scaling.
- **D is correct.** `q · k1 = (1)(1) + (1)(0) = 1`, `q · k2 = (1)(0) + (1)(1) = 1`. Scaling both by `1/sqrt(2)` keeps them equal, and softmax over two equal values always splits 50/50.
- **B is incorrect.** This is a plausible-looking unequal split, but the two raw scores are exactly equal — there's no basis for an uneven weight here.
- **C is incorrect.** The weights come entirely from Q and K; V only determines what those weights get multiplied against afterward, not the weights themselves. See [Scaled Dot-Product Attention in Numpy](/learn/llm-foundations/scaled-dot-product-attention-in-numpy).
...
A. Because dot products of higher-dimensional vectors have larger variance, which pushes softmax toward saturated, near-one-hot outputs with vanishing gradients if left unscaled
B. To force every score to be positive before softmax runs
...
**Correct: A.** The variance of a dot product of `d_k` independent unit-variance terms grows with `d_k`; dividing by `sqrt(d_k)` cancels that growth and keeps the logit variance at 1 regardless of dimension.

- **B is incorrect.** Softmax handles negative inputs fine — scaling isn't about sign, it's about magnitude and variance.
- **A is correct.** See the full derivation and numeric example in [Why Divide by the Square Root of d_k](/learn/llm-foundations/why-divide-by-sqrt-dk).
...
A. 512 per head; about 8x more total compute
B. 64 per head; about 8x more total compute, since each head redoes the full computation
C. 64 per head; roughly the same total compute as one wide head
D. 8 per head; roughly the same total compute as one wide head

<details>
<summary>Answer</summary>

**Correct: C.** `512 / 8 = 64` per head, and because each head does proportionally less work over a proportionally smaller dimension, the total compute across all 8 heads stays close to what one full-width head would cost.

- **C is correct.** See [Multi-Head Attention: Why Many Heads](/learn/llm-foundations/multi-head-attention-why-many-heads) for why splitting doesn't multiply cost the way it multiplies the number of independent attention patterns.
- **A is incorrect.** This mixes up the per-head dimension with the full model dimension — heads operate on slices, not the full width each.
- **B is incorrect.** Each head operates on a smaller slice, not a redundant full copy — the per-head cost shrinks along with the per-head dimension.
...
B. The feed-forward block only runs during training, not at inference
C. Attention has learned parameters, while the feed-forward block does not
D. Attention mixes information across positions; the feed-forward block processes each position independently, with the same weights applied to every token

<details>
<summary>Answer</summary>

**Correct: D.** Attention is the only sublayer where one token's output can depend on another token's input; the FFN runs the identical two-layer MLP on every position independently, with zero cross-position mixing.
...
- **D is correct.** See [The Feed-Forward Block and Its Role](/learn/llm-foundations/the-feed-forward-block-role) for the "attention moves, FFN processes" framing.
- **B is incorrect.** The FFN runs at both training and inference, exactly like attention — every forward pass uses it.
- **C is incorrect.** Both sublayers have learned parameters — attention's Q/K/V/output projections, and the FFN's two (or three, for SwiGLU) weight matrices.
...
<Callout kind="try" title="Work through them before checking each answer">
This module builds end to end, from queries, keys, and values through causal masking, multi-head splitting, the feed-forward block, and residual/norm placement. If a question here took more than one pass, the linked lesson in that answer is the place to re-derive it, not just re-read it.
</Callout>
