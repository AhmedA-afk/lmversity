...
<Steps>
  <Step title="Normalize every row to unit length">
...
**Why this step?** Cosine similarity measures the *angle* between two vectors, not how long they are. Two embeddings pointing in nearly the same direction should count as similar even if one happens to have a larger magnitude — normalizing first is what makes the dot product that follows measure angle alone. See [cosine similarity](/learn/maths-foundations/cosine-similarity-angular-distance-embedding-retrieval) for why angle, specifically, is the right notion of "similar" here.
  ...
  <Step title="Compute every similarity at once with a single matmul">
...
**Why this step?** `E_unit @ E_unit[idx]` computes the dot product of the query token's normalized vector against *every* row of the matrix in one operation — because both sides are unit vectors, that dot product is exactly the cosine similarity. This is the real mechanism behind nearest-neighbor search at any scale: one matrix-vector product against the whole table, not a loop comparing pairs one at a time.
  ...
  <Step title="Rank neighbors for dog, cat, and car">
...
<InlineCheck
  question="Why does the normalize step run before the matmul that computes similarity, rather than comparing the raw rows of E directly?"
  options={['Normalizing makes the numbers smaller and therefore faster to multiply', 'Cosine similarity measures the angle between vectors, and normalizing to unit length is what makes a dot product equal that angle rather than being skewed by vector length', 'It converts the embeddings from floats into integers for exact comparison']}
  answer={1}
  explanation="Cosine similarity is about direction, not magnitude. Normalizing every row to unit length is exactly what makes the dot product that follows measure angle alone."
/>

## Where it breaks (+fix)

<Callout kind="breaks" title="Raw dot products let magnitude masquerade as meaning">
...
<Callout kind="production" title="Always compare normalized vectors for nearest-neighbor search">
...
<InlineCheck
  question="A raw (unnormalized) dot product ranks a long, weakly-related vector above a short, strongly-aligned one. What does this demonstrate?"
  options={['That cosine similarity and raw dot product always agree on ranking', 'That raw dot product can confuse a vector’s length with how well-aligned it is, which normalizing before comparing corrects', 'That the embedding matrix must be recomputed to fix the ranking']}
  answer={1}
  explanation="The spam vector wins on raw dot product purely because it is long, not because it points toward dog. Normalizing to unit length before comparing removes magnitude from the comparison, leaving only direction."
/>
