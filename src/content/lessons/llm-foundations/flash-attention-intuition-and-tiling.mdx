...
<Step title="Write the naive baseline">
...
<Callout kind="note">
We need ground truth to check our tiled version against. Notice the full `n x n` `scores` matrix exists in memory here — for n = 8 that's trivial, but it's exactly this array that becomes the 64 GiB monster at 128k context described in [The Quadratic Attention Bottleneck](/learn/llm-foundations/the-quadratic-attention-bottleneck).
</Callout>

</Step>

<Step title="Write the tiled, online-softmax version">
...
<Callout kind="note">
At no point does this function hold more than one query vector and one block of keys/values at a time — never the full sequence, never the full score matrix. The `correction` term is the trick: because softmax's max-subtraction is just for numerical stability, and the max can change as new, larger scores arrive, each new block rescales the running total to what it *would have been* if this new block's max had been known from the start. Provided the arithmetic is right, this produces a bit-for-bit-equivalent (up to floating point rounding) result to computing the whole row of scores at once.
</Callout>
...
<InlineCheck
  question="Changing block_size from 3 to 1 or 8 in the tiled implementation does not change the output. Why not?"
  options={['Block size only changes how the computation is chunked, never what it computes, thanks to the online-softmax correction term', 'The block_size parameter is unused in the actual computation', 'The outputs do change slightly, but the difference is too small to matter']}
  answer={0}
  explanation="The correction term rescales the running total whenever a new block's max exceeds the previous running max, so the final result is mathematically the same regardless of how the keys and values are chunked into blocks."
/>

## Harden it

<Callout kind="production" title="What a production kernel adds">
A few things a production kernel does that this toy version simplifies away: it tiles Q as well as K/V, processing blocks of queries together rather than one query at a time, so the whole computation runs as dense matrix multiplications on hardware. During the backward pass, instead of storing the full n × n matrix for backward (defeating the whole point), it recomputes the needed blocks, trading a bit of extra compute for the memory savings. And even if Q, K, V are stored in fp16 or bf16, the running `m`, `l`, and `acc` accumulators are kept in fp32 — the online-softmax correction involves repeated multiplication by correction factors, and low-precision accumulation here would compound rounding error across blocks.
</Callout>
...
It's worth being precise about what this technique does and doesn't buy you: it does not reduce the number of floating-point operations attention performs — the same Q·Kᵀ dot products still happen. What it reduces is memory traffic: the full score matrix never gets written to and read back from slow memory, only small blocks pass through fast on-chip memory at a time. That's a different lever from [Sparse, Sliding-Window, and Linear Attention](/learn/llm-foundations/sparse-sliding-and-linear-attention), which changes *which* token pairs get computed at all, or [From MHA to MQA: Why Grouped-Query Attention Won](/learn/llm-foundations/multi-query-and-grouped-query-attention), which changes how much needs to be cached and re-read across generation steps.

<InlineCheck
  question="Does FlashAttention reduce the number of floating-point operations attention performs?"
  options={['Yes, it skips computing scores for most token pairs', 'No — the same Q·Kᵀ dot products still happen; what it reduces is memory traffic, not FLOPs', 'Yes, because it uses lower-precision arithmetic']}
  answer={1}
  explanation="FlashAttention computes every token pair, the same as naive attention. What changes is that the full score matrix never gets written to and read back from slow memory — the savings are in memory traffic, not in the FLOP count."
/>
