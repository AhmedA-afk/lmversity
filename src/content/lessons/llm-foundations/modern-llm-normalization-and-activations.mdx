...
<Step title="Swap 1: LayerNorm to RMSNorm">
...
<Step title="Swap 2: GeLU feed-forward to SwiGLU gated feed-forward">
...
<Step title="Swap 3: sinusoidal or learned positions to RoPE">
...
<Step title="Swap 4: dropping bias terms">
...
<Callout kind="production" title="Justified by scale, not by derivation">
The empirical case reported by teams training at this scale (the PaLM paper is a documented example) is that removing bias terms had negligible effect on quality while measurably improving training stability at large scale — fewer terms that can drift or interact awkwardly with the optimizer and with distributed-training setups, for a small saving in parameter count as a side benefit. This is a case where the change is justified almost entirely by "we tried it at scale and it worked better," rather than a derivation from first principles — worth being honest about, since not every modern default has a clean theoretical story behind it.
</Callout>
...
<InlineCheck
  question="Why does a SwiGLU feed-forward block typically use a narrower hidden dimension than the '4x d_model' rule of thumb from the original architecture?"
  options={['Because SwiGLU is a weaker nonlinearity that needs less capacity', 'Because SwiGLU needs three weight matrices instead of two, so the hidden dimension is shrunk to roughly (8/3)·d_model to keep the total parameter count matched', 'Because narrower hidden dimensions train faster, independent of any parameter-count matching']}
  answer={1}
  explanation="SwiGLU adds a third weight matrix (W1, W2, W3) compared to a plain FFN's two. To keep the parameter count matched at 8·d_model², the convention shrinks the hidden dimension to roughly (8/3)·d_model — about two-thirds of the original '4x' width."
/>

## Putting it together

<Compare title="2017 transformer block vs modern transformer block">
  <Pane title="2017 block" tone="bad">
...
```
  ...
  <Pane title="Modern block" tone="good">
...
<InlineCheck
  question="What is the main empirical justification given for removing bias terms in modern large models?"
  options={['A mathematical proof that bias terms are redundant given RMSNorm', 'Teams training at scale (e.g. the PaLM paper) reported negligible quality effect and measurably improved training stability — an empirical finding, not a first-principles derivation', 'Bias terms were found to leak information across attention heads']}
  answer={1}
  explanation="The case for dropping bias terms is empirical: teams training at this scale reported negligible effect on quality and improved training stability, plus a small parameter saving. It's explicitly a 'we tried it and it worked' finding rather than a theoretical derivation."
/>
