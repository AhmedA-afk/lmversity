...
<Tokenizer text="🎉 👨‍👩‍👧‍👦 12345 https://lmversity.com/learn/llm-foundations kütüphanecilerimizden" />

## Step by step

<Steps>
  <Step title="🎉 — a single common emoji">
...
This is the case people wrongly generalize from. One familiar emoji costing one token creates the intuition that "emoji are cheap," which the next example breaks immediately.
  </Step>
  <Step title="👨‍👩‍👧‍👦 — a family emoji built from a ZWJ sequence">
...
Visual unit and token unit are unrelated. The tokenizer has no concept of "one emoji" — it only has bytes and its learned merges, and a compound glyph is, underneath, a run of several distinct codepoints.
  </Step>
  <Step title="12345 — a plain five-digit number">
...
Punctuation-dense, low-frequency strings are exactly what subword tokenization is worst at compressing — vocabulary slots got spent on frequent natural-language substrings, and a specific URL wasn't one of them. This is also why pasting file paths, hashes, or IDs into a prompt eats context faster than the visible text length suggests.
  </Step>
  <Step title="kütüphanecilerimizden — a Turkish word">
...
Tokenization isn't language-neutral. A vocabulary trained mostly on English text encodes English most efficiently by construction — every other language pays a real, measurable tax in tokens-per-word, which shows up directly as more expensive prompts and less usable context for non-English text.
  </Step>
</Steps>

<InlineCheck
  question="Why does the family emoji 👨‍👩‍👧‍👦 typically cost several tokens instead of one?"
  options={['It is actually several base emoji joined by invisible zero-width joiner characters, and the tokenizer sees the underlying bytes, not the rendered glyph', 'The tokenizer has a special rule that always splits emoji into individual people', 'Emoji are never included in a byte-level BPE vocabulary']}
  answer={0}
  explanation="A tokenizer only has bytes and its learned merges — it has no concept of “one emoji.” A compound glyph like this is, underneath, a run of several distinct codepoints joined by ZWJ characters, so it fragments accordingly."
/>

<InlineCheck
  question="Why does “12345” typically split into two tokens like “123” and “45” instead of one?"
  options={['Byte-level BPE tokenizers cap how many consecutive digits can fuse into one pretoken, commonly at three, to keep arithmetic from getting worse', 'The tokenizer rounds numbers longer than four digits', 'Digits are never merged with other digits under any circumstance']}
  answer={0}
  explanation="The digit-count cap is a deliberate design choice, not an accident — letting merges group digits arbitrarily made arithmetic worse, so the split boundary is decided by that cap, with no regard for place value."
/>

## Where it breaks (+fix)

<Callout kind="breaks" title="A prompt that looks short can still cost far more than it looks">
...
<Callout kind="production" title="Count tokens with the real tokenizer">
