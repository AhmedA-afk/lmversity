...
<Callout kind="breaks" title="Most exposed to reward hacking">
PPO-based RLHF is unstable and finicky to tune (KL coefficient, reward scale, RL hyperparameters), and most exposed to reward hacking, since it actively searches for whatever maximizes reward-model score, including exploits the reward model never anticipated.
</Callout>
...
<Compare title="PPO-based RLHF vs. DPO">
  <Pane title="PPO-based RLHF" tone="neutral">
    Samples on-policy from the model as it currently is, so it can optimize against reward for responses the model actually produces now. Unstable and finicky to tune, and most exposed to reward hacking.
  </Pane>
  <Pane title="DPO" tone="neutral">
    Trains on a fixed set of (chosen, rejected) pairs with a closed-form loss — no reward model, no RL loop. Can't correct for preferences about responses the current policy wouldn't have generated in the first place.
  </Pane>
</Compare>

<InlineCheck
  question="What does DPO give up by skipping the reward model and the online RL loop?"
  options={['On-policy correction — since it trains only on the fixed pairs it was given, it can’t correct for preferences about responses the current policy wouldn’t have generated in the first place', 'The ability to use a KL penalty at all', 'Any connection to human preference data']}
  answer={0}
  explanation="DPO is off-policy: it trains on a static dataset of chosen/rejected pairs rather than samples from the current policy. PPO's on-policy sampling is what lets it optimize against reward for responses the model is producing right now."
/>
...
<Callout kind="production" title="Start with DPO, add machinery only when the failure mode shows up">
