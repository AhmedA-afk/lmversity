...
<Callout kind="breaks" title="A single scale factor can waste range on rare outliers">
Some weight distributions have a few outlier values with much larger magnitude than the rest, and a single scale factor calibrated to cover those outliers wastes most of the 8-bit range on values that never get that large — degrading precision for the typical weight to accommodate the rare one. Per-channel scaling mitigates this but adds bookkeeping.
</Callout>
...
<InlineCheck
  question="What specifically makes GPTQ's 4-bit quantization practical, where naive 4-bit rounding degrades quality much more sharply?"
  options={['GPTQ retrains the full model from scratch at 4-bit precision', 'GPTQ quantizes layer by layer and compensates for rounding error already introduced by previously-quantized weights, using second-order information to choose which weights absorb error best', 'GPTQ simply uses a smaller calibration dataset than INT8']}
  answer={1}
  explanation="GPTQ's error-compensation scheme — quantizing layer by layer while accounting for error already introduced, guided by second-order information — is specifically what makes 4-bit practical. Naive rounding without it degrades quality much more sharply."
/>
...
<Callout kind="production" title="These five stack; they are not alternatives to pick between">
Production stacks typically combine all five techniques, because they act on different resources — continuous batching should be the default for any concurrent serving, not an optimization saved for later.
</Callout>
...
<InlineCheck
  question="A team wants to serve real production traffic with variable-length requests arriving continuously. According to the decision table and the guidance above, what should be the default, not an optional later step?"
  options={['INT4 quantization, since maximum compression always comes first', 'Continuous batching, since it targets exactly this traffic pattern and nearly every modern serving engine provides it', 'Running requests one at a time to guarantee the lowest possible per-request latency']}
  answer={1}
  explanation="Continuous batching wins on any real production traffic pattern where requests arrive continuously and vary in length — the guidance is explicit that it should be the default, not an optimization considered later."
/>
