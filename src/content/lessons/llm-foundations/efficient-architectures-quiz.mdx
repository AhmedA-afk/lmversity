...
**Correct: B.** The score matrix is n × n, so a 4x increase in sequence length produces a 4² = 16x increase in matrix size.
...
- C) Not derived from anything in the setup — there's no operation here that produces an 8x factor.
- B) Correct — quadratic growth means squaring the growth factor of the input: 4² = 16.
...
A) There are 8 distinct K/V pairs, and the model performs roughly 8x fewer attention FLOPs than full multi-head attention
B) There is only 1 shared K/V pair, used by all 32 query heads
C) There are 8 distinct K/V pairs, each shared by 4 query heads, and attention FLOPs are roughly unchanged from full multi-head attention
D) GQA reduces the number of query heads from 32 down to 8

<details>
<summary>Answer</summary>

**Correct: C.** Grouping reduces the number of distinct key/value sets, which shrinks the KV cache and the K/V projection cost — but each of the 32 query heads still computes its own dot products against its group's keys over the full sequence, so the dominant attention FLOPs (the Q·Kᵀ and softmax·V steps) are essentially unaffected by grouping.

- C) Correct — this is the core distinction the whole module draws: GQA is a memory and bandwidth optimization, not primarily a FLOPs optimization.
- A) This is the common misconception the correct answer corrects — fewer distinct K/V sets doesn't mean fewer query-to-key comparisons; there are still 32 query heads each attending over the sequence.
- B) That describes MQA (group count = 1), not GQA with 8 groups.
...
**Correct: D.** `elements = 2 (K and V) × 2 layers × 4 KV heads × 8 head_dim × 1,024 tokens = 131,072`; `bytes = 131,072 × 2 (fp16) = 262,144 bytes = 256 KiB`.

- D) Correct — the formula uses `num_kv_heads` (4), not the query head count.
- A) This is what you get if you mistakenly plug in the 32 query heads instead of the 4 KV heads: `2 × 2 × 32 × 8 × 1,024 × 2 = 2,097,152 bytes = 2 MiB`. It's the exact error tested in Question 2 — conflating query heads with KV heads.
- B) This is what you get by using fp32 (4 bytes) instead of the stated fp16 (2 bytes): `131,072 × 4 = 524,288 bytes = 512 KiB`. Always match the precision given in the problem.
- C) This is what you get by forgetting the factor of 2 for storing both K *and* V: `1 × 2 × 4 × 8 × 1,024 × 2 = 131,072 bytes = 128 KiB` — half the correct answer.
...
A) The exact same softmax attention output, just computed in blocks with less memory traffic
B) An approximation of softmax attention, trading a small accuracy loss for speed
...
**Correct: A.** The online-softmax recurrence produces bit-for-bit-equivalent results (up to floating-point rounding) to the naive computation — it changes how the computation is organized in memory, not what it computes.

- B) This is the most common misconception about FlashAttention — it is exact, not an approximation. Nothing about the math it performs differs from naive attention.
- A) Correct — this is the "IO-aware, not approximate" point stressed throughout [FlashAttention: The Tiling and Online-Softmax Idea](/learn/llm-foundations/flash-attention-intuition-and-tiling).
...
<Callout kind="note" title="Two different bottlenecks, easy to conflate">
Several of these questions turn on keeping two different bottlenecks apart: the quadratic attention-score cost (Q1, Q7) versus the linear KV-cache cost (Q2, Q3), and MoE's active-compute cost versus its total-memory cost (Q6). Missing an answer here usually means those two costs got merged into one in your head.
</Callout>

**Related:** [The Quadratic Attention Bottleneck](/learn/llm-foundations/the-quadratic-attention-bottleneck), [From MHA to MQA: Why Grouped-Query Attention Won](/learn/llm-foundations/multi-query-and-grouped-query-attention), [Mixture of Experts: Routing](/learn/llm-foundations/mixture-of-experts-routing), [MoE Load Balancing and Its Failure Modes](/learn/llm-foundations/moe-load-balancing-and-failure-modes)
