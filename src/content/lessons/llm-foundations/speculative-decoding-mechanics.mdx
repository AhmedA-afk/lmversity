...
<InlineCheck
  question="A draft proposes a token with q(x) = 0.5. The target assigns it p(x) = 0.5. What is the acceptance probability?"
  options={['1 — since p(x) ≥ q(x), the token is accepted outright', '0.5 — accept(x) always equals p(x) regardless of q(x)', '0 — the token is rejected because the draft and target agree exactly']}
  answer={0}
  explanation="The accept rule is min(1, p(x)/q(x)). When p(x) equals q(x), that ratio is 1, so accept(x) = min(1, 1) = 1 — accepted with certainty, the same as any case where the target agrees the token was at least as likely as the draft thought."
/>
...
<InlineCheck
  question="A proposed token y has q(y) = 0.5 and the target assigns p(y) = 0.2. What is the acceptance probability, and what happens on rejection?"
  options={['Acceptance is min(1, 0.2/0.5) = 0.4; on rejection the replacement is drawn from max(0, p - q) across the vocabulary, not a uniform re-roll', 'Acceptance is 0.5, matching the draft’s own probability; on rejection the target falls back to greedy decoding', 'Acceptance is 1, since the target still assigned the token a nonzero probability']}
  answer={0}
  explanation="The accept rule is min(1, p/q) = min(1, 0.2/0.5) = 0.4. A rejection doesn't fall back to greedy or a uniform re-roll — it resamples from max(0, p - q), the specific corrected distribution that reconstructs p exactly."
/>
...
<Callout kind="breaks" title="Assuming any two models can be paired as draft and target">
A draft that's a poor stylistic match — different tokenizer, very different training distribution — gets rejected constantly, and the guarantee of correctness still holds, but the speedup evaporates because so little survives verification.
</Callout>

<Callout kind="breaks" title="Forgetting rejection cascades">
One rejected token invalidates every proposed token after it in that round, even ones that might have been individually fine — the draft's proposal for token `k+2` was built assuming its guess for token `k+1` was accepted.
</Callout>

<Callout kind="breaks" title="Treating the resample step as an afterthought">
Skipping the `max(0, p - q)` correction and just falling back to sampling from `p` alone after a rejection seems harmless but subtly biases the output, because it double-counts the probability mass where `p` and `q` already agreed.
</Callout>
