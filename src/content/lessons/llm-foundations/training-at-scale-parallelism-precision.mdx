...
You can understand what pretraining optimizes ([The Pretraining Objective and Its Loss](/learn/llm-foundations/the-pretraining-objective-and-loss)) and how the optimizer updates weights ([Optimization Mechanics](/learn/llm-foundations/optimization-mechanics-adam-warmup)) without this lesson. This is the engineering underneath a large run — worth knowing so "trained on many GPUs" means something concrete.
</Callout>
...
<InlineCheck
  question="A model's largest weight matrices no longer fit in a single GPU's memory even with just one copy of the model. Which axis of parallelism directly addresses that?"
  options={['Tensor parallelism, which splits individual weight matrices across GPUs so no single device needs a full copy of the largest layers', 'Data parallelism, which just processes different batch slices on full model copies', 'Pipeline parallelism, which only splits the model by depth, not by the size of any one layer']}
  answer={0}
  explanation="Data parallelism assumes the model already fits on one device — it doesn't help if a single copy is too big. Pipeline parallelism splits by depth, not by an individual layer's size. Tensor parallelism is the one that shards a weight matrix itself across devices."
/>
...
<InlineCheck
  question="Why is tensor parallelism usually kept within a single node, over the fastest interconnect, while data parallelism can span the whole cluster?"
  options={['Tensor parallelism communicates on every layer, so it needs the fastest links; data parallelism only needs to communicate once per step, for the gradient average, so slower links are tolerable', 'Data parallelism requires more total bandwidth than tensor parallelism', 'Tensor parallelism does not require any communication between devices']}
  answer={0}
  explanation="Tensor parallelism shards a matrix multiply and has to combine partial results within every layer, which demands the fastest links. Data parallelism only synchronizes gradients once per step, which tolerates a slower network fabric."
/>

## Tradeoffs to hold in tension

<Callout kind="production" title="Three tradeoffs to hold in tension">
