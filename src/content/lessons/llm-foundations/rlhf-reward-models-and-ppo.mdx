...
<Steps>
  <Step title="Freeze a reference model">The SFT model serves as the starting policy, and a frozen copy of it also serves as the reference model for the rest of training.</Step>
  <Step title="Train a reward model on preference pairs">A reward model is trained on human preference comparisons: given a prompt and two candidate responses, predict which one a human preferred.</Step>
  <Step title="Run PPO against the reward model, anchored to the reference">PPO (Proximal Policy Optimization), a reinforcement-learning algorithm, updates the policy so it generates responses the reward model scores highly — subject to a KL-divergence penalty that keeps the policy's output distribution close to the frozen reference model.</Step>
</Steps>
...
<Callout kind="breaks" title="A reward model is only ever a proxy">
Without a leash back to the reference model, PPO will find and exploit gaps between what the reward model rewards and what humans actually prefer — this is reward hacking. A reward model is only ever a proxy for human preference, trained on a finite pool of raters and prompts, and optimizing against it too hard, for too long, reliably finds its blind spots.
</Callout>

<InlineCheck
  question="Why does PPO subtract a KL penalty from the reward-model score instead of optimizing that score directly?"
  options={['Because the reward model is only an approximation of human preference, and without a penalty for drifting from the reference model, PPO will find and exploit gaps between what the reward model rewards and what humans actually want', 'Because the KL penalty makes the reward model’s scores more accurate estimates of human preference', 'Because PPO cannot compute a policy update at all without a KL term in the objective']}
  answer={0}
  explanation="The reward model is a proxy trained on a finite comparison set. Without the KL penalty anchoring the policy to the frozen reference, PPO is free to chase reward-model score in ways that diverge from what humans actually prefer — reward hacking."
/>
...
<InlineCheck
  question="What does PPO's clipped objective actually limit?"
  options={['How much the ratio of new-to-old action probabilities can influence a single update, so one noisy batch can’t swing the policy too far in one step', 'The number of preference pairs the reward model can be trained on', 'The maximum KL divergence between the policy and the reward model’s own output distribution']}
  answer={0}
  explanation="PPO clips how far a single update can move the ratio of new-to-old action probabilities. That bounds the effect of any one batch's reward estimate, which is what keeps naive policy-gradient updates from overcorrecting and destabilizing training."
/>
...
<Callout kind="production" title="PPO-based RLHF is expensive and fiddly relative to its alternatives">
It needs a working reward model, a stable RL loop, and careful tuning of the KL coefficient. See [RLHF vs DPO vs Other Preference Methods](/learn/llm-foundations/rlhf-vs-dpo-vs-preference-methods) for when a simpler method gets most of the benefit.
</Callout>

<Callout kind="production" title="The KL coefficient is a real dial with a real tradeoff">
Too small and the policy drifts and reward-hacks, too large and it barely moves from the SFT model, wasting the stage.
</Callout>
