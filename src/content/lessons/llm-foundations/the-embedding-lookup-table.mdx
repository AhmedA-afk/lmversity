...
<InlineCheck
  question="What computation happens when a model 'looks up' a token's embedding?"
  options={['A small trained sub-network processes the token ID and outputs a vector', 'No computation at all — the token ID is used to index a row of a pre-built table', 'A softmax is run over every row of the table to find the best match']}
  answer={1}
  explanation="Looking up an embedding is just indexing: the token ID selects a row number, and that row's numbers — set once ahead of time by training — are returned unchanged. There is no transformation involved."
/>
...
<InlineCheck
  question="Why does one_hot(3) @ E give the same result as indexing E[3] directly?"
  options={['Because one-hot vectors always have length 1', 'Because the one-hot multiply zeroes out every row except the matching one, then sums — leaving that one row unchanged', 'Because E is a square matrix']}
  answer={1}
  explanation="Multiplying a one-hot vector by a matrix zeroes out every row except the one matching the 1 and sums the result. With only one nonzero entry, that sum is just the matching row, unchanged — identical to indexing it directly."
/>
...
<Callout kind="breaks" title="The rows are learned, not assigned by meaning">
Nobody decides "dog" gets values `[0.44, 0.12, ...]` — those numbers start random and are shaped entirely by [pretraining](/learn/llm-foundations/pretraining-explained), the same gradient updates that shape every other weight in the model. Row *position* (which token ID gets which row index) is arbitrary and fixed at vocabulary-build time; row *content* is the only part that's learned.
</Callout>
...
<Callout kind="note" title="Tied weights don't mean two independent matrices">
