...
<InlineCheck
  question="Why does prompting's cost scale differently with volume compared to fine-tuning?"
  options={['Because every instruction and example is re-sent and re-billed on every single call, so cost grows with call volume', 'Because prompting requires a one-time training run whose cost is amortized over volume', 'Prompting and fine-tuning scale identically with volume; the difference is only in setup time']}
  answer={0}
  explanation="Prompting has no training run, so its cost shows up entirely at inference time — the full instruction and examples get re-sent (and re-billed) on every call, unlike fine-tuning's upfront cost and low marginal cost per query."
/>
...
<InlineCheck
  question="A RAG system produces a hallucinated answer even though the generation model is strong. What does this lesson say is the most likely bottleneck?"
  options={['The generation model needs to be fine-tuned instead', 'Retrieval quality — an irrelevant or missing chunk produces a bad answer regardless of how good the generation model is', 'RAG systems cannot hallucinate by design, so this must be a prompting issue']}
  answer={1}
  explanation="RAG's whole system is bottlenecked by retrieval quality. If the retrieved chunk is irrelevant or missing, the model is conditioning its answer on bad or absent evidence, independent of how capable the generation model itself is."
/>
...
<Compare title="Is the problem knowledge, or is it behavior?">
  <Pane title="Knowledge problem" tone="good">
    The model doesn't know something (or knows an outdated version of it) — reach for RAG.
  </Pane>
  <Pane title="Behavior problem" tone="good">
    The model knows enough but won't reliably say it the way you need (tone, structure, refusal behavior) — reach for fine-tuning.
  </Pane>
</Compare>

<Callout kind="production" title="Always try prompting first">
Is it worth solving with training infrastructure at all yet? Always try prompting first, even for a problem you suspect needs more — [few-shot vs. zero-shot: worked prompts](/learn/llm-foundations/few-shot-vs-zero-shot-worked) shows how much can be pinned down without touching weights or a vector store, and it's the cheapest way to discover you actually need one of the heavier options before you build it.
</Callout>
