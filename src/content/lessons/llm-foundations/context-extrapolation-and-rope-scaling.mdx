...
<InlineCheck
  question="Why are the slow-rotating (high-index) dimension pairs the real casualty of naive RoPE extrapolation?"
  options={["They never came close to covering their full angular range during training, so extending the sequence pushes them into phase values the model never learned to interpret", "They rotate so fast that they alias back onto already-familiar angles", "They are not used by the attention dot product at all"]}
  answer={0}
  explanation="Fast-rotating pairs cycle through their range many times even during training, so extrapolation mostly revisits familiar angles. Slow-rotating pairs never got near a full rotation, so every offset past the trained length lands outside the narrow slice they actually learned."
/>
...
<Callout kind="breaks" title="Compression isn't free">
...
<InlineCheck
  question="What does linear scaling (position interpolation) actually change about how a rotation angle is computed?"
  options={["It feeds position × (L_train / L_new) into the rotation formula instead of the raw position, so every angle stays within the trained range", "It increases the model's base frequency so angles rotate faster", "It removes RoPE for positions beyond L_train and switches to no positional signal at all"]}
  answer={0}
  explanation="Linear scaling compresses every real position by a fixed factor before it reaches the rotation formula, so no matter how long the actual input is, the angle computed never exceeds what the model saw during training."
/>
