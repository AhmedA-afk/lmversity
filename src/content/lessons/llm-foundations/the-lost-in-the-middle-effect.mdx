...
<InlineCheck
  question="What shape does the recall-versus-position curve take when a fact is planted at different points in a long context?"
  options={['A straight decline as the fact moves later in the context', 'A U-shape — high at both the start and the end, lowest in the middle', 'Flat — position has no measurable effect on recall']}
  answer={1}
  explanation="Both ends of the context do well and the middle is the weak point, which is why the curve is a U rather than a ramp. A pure recency effect would only explain the end doing well, not the start."
/>
...
<InlineCheck
  question="Which single mechanism, on its own, would predict only a decline toward the middle and end — with no explanation for why the very start also performs well?"
  options={['Mechanism one: attention sinks', 'Mechanism two: positional schemes biasing toward recency', 'Mechanism three: training-length distribution']}
  answer={1}
  explanation="Positional decay alone favors whatever is closest to the generation point, which is the end of the context — it has no reason to favor the start. The start's strength comes from mechanism one, the attention sink, stacking on top of it."
/>
...
<Callout kind="production" title="Put critical instructions at an edge, not the middle">
...
<Callout kind="production" title="Rerank retrieved passages so relevance lands at an edge">
...
<Callout kind="production" title="Consider stating a critical fact twice">
For long documents where one fact matters most, consider stating it twice — once near the top as a preview or summary, once in its natural position — rather than relying on a single placement in an unfavorable spot. Don't treat "it's inside the context window" as equivalent to "the model will use it well." [Context window mechanics and limits](/learn/llm-foundations/context-window-mechanics-and-limits) covers what determines whether something fits; this effect governs something different — how reliably the model actually draws on what fits.
</Callout>

## The one thing this doesn't mean

<Callout kind="breaks" title="Not invisible, just less reliable">
