...
<InlineCheck
  question="A task requires getting all k independent sub-decisions right, and per-sub-decision accuracy p improves smoothly with scale. Why can the exact-match score (p^k) still look like a sudden cliff?"
  options={['Because raising a smoothly increasing number to a fixed power k produces a curve that stays flat near zero for a while, then rises steeply, purely from the exponent', 'Because p itself must be discontinuous for p^k to look like a cliff', 'Because exact-match scoring rounds p up or down at a hidden threshold before computing p^k']}
  answer={0}
  explanation="The cliff shape comes purely from raising a smooth p to the power k — no discontinuity in p is required. That is the entire mechanism behind the mirage argument."
/>
...
<InlineCheck
  question="In the code example, the per-digit accuracy p is identical across all three rows (k=1, k=5, k=20). What differs between the rows, and why?"
  options={['Only the scoring rule (the exponent k) differs — the underlying model improvement is identical in all three rows', 'The underlying model actually improves faster at higher k', 'The rows use different accuracy curves, so the comparison is not meaningful']}
  answer={0}
  explanation="The example is built from one single p curve; only k changes across rows. The sharper cliff at higher k is entirely a scoring-rule artifact, not a different underlying trend."
/>
...
<Callout kind="breaks" title="An artifact isn't something to ignore">
...
<Callout kind="breaks" title="A continuous substitute isn't always available">
...
<Callout kind="breaks" title="Smooth at your k today doesn't mean smooth at k+1">
