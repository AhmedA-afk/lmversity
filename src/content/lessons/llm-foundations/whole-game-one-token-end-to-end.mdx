...
<InlineCheck
  question="A token's hidden state enters a transformer block with shape (5, 768). What shape does it have on the way out?"
  options={['(5, 768) — the same shape, with new information mixed in', '(5, 3072), because the FFN up-projects it', '(1, 768), because attention collapses the sequence to one vector']}
  answer={0}
  explanation="A block's output shape matches its input shape, (5, 768) in and (5, 768) out — the FFN's up-projection to (5, 3072) happens inside the block but is projected back down before the block returns its result."
/>
...
<InlineCheck
  question="After a token is sampled in step 8, what does step 9 do with it?"
  options={['Discards it and starts over from the original prompt', 'Appends it to the sequence and feeds the whole thing back in as input for the next forward pass', 'Uses it to update the model weights before continuing']}
  answer={1}
  explanation="The loop appends the sampled token to the sequence and feeds it back in as input, going back to step 1 — that append-and-refeed step is what turns one next-token prediction into a paragraph instead of a single word."
/>

<Callout kind="note" title="No step where the model 'thinks' outside this pipeline">
...
**Related:** [The Transformer Architecture](/learn/llm-foundations/the-transformer-architecture), [Next-Token Prediction](/learn/llm-foundations/next-token-prediction), [Sampling, Temperature, and Top-p](/learn/llm-foundations/sampling-temperature-top-p); [RAG, End to End: The Whole Game](/learn/rag/rag-whole-game)
