...
<InlineCheck
  question="Why does the guesser end up picking up biology-flavored associations, code syntax, and arithmetic-shaped patterns, when none of those were the stated assignment?"
  options={['The training data included explicit labels marking which passages teach biology, code, or arithmetic', 'Getting a low loss on the one instruction — predict what comes next — across a corpus this diverse requires internalizing that structure anyway', 'The guesser is separately retrained on each domain after the main prediction game is finished']}
  answer={1}
  explanation="The only objective at every step was guess the next token, but achieving low loss on that narrow objective, across text this diverse, quietly requires internalizing structure that looks like knowledge, grammar, and reasoning — because that structure is what makes prediction cheaper and more confident."
/>

## The wrong intuition, corrected

<Callout kind="note" title="Nobody wrote a training example labeled 'learn arithmetic'">
...
<Callout kind="breaks" title="More internet text does not mean more truth">
...
<InlineCheck
  question="Why doesn't feeding a model more internet text automatically mean it learns more true things?"
  options={['Because self-supervised learning only works on text that has already been fact-checked', 'Because misinformation and confident-sounding nonsense help predict what comes next exactly as efficiently as true statements do', 'Because larger corpora force the vocabulary size to shrink']}
  answer={1}
  explanation="Self-supervision means no labels are needed, not that the implicit labels are always correct. The internet teaches misinformation and stylistic bias exactly as efficiently as it teaches anything true, whenever that content helps predict what comes next in its own context."
/>

## When the analogy breaks

<Callout kind="breaks" title="No channel into the world besides text statistics">
