...
<Callout kind="production" title="If you can't check every box">
Ship what's true and mark the rest as a known gap — an honest gap list is itself evidence you understand what each piece does.
</Callout>

<InlineCheck
  question="How does the acceptance list say you should verify that causal masking is actually working?"
  options={["Confirm position i's output is unchanged when you alter tokens at positions > i", "Check that the loss curve is smooth with no bumps", "Compare the model's output against a reference implementation's output byte for byte"]}
  answer={0}
  explanation="The acceptance criteria call for confirming that changing a later token has no effect on an earlier position's output — the direct behavioral test that the mask is actually blocking future positions."
/>
...
<Steps>
  <Step title="The pipeline runs end to end, badly">
Random-initialized model, one forward pass, one backward pass, loss is a number (even a bad one). This is the milestone that catches shape bugs before you've wasted a training run on them.
  </Step>
  <Step title="Loss actually falls">
A real training run, logged, shows a clearly decreasing loss curve — not necessarily smooth, but trending down over hundreds to thousands of steps.
  </Step>
  <Step title="Generations go from noise to structure">
Sample text at the start of training (should look like random tokens), partway through (should look like plausible local structure — real words, rough grammar), and near the end (should look like short, locally coherent passages in the style of your corpus).
  </Step>
  <Step title="You can explain every shape in the forward pass">
Given any intermediate tensor in your model, you can state its shape and why it's that shape, without looking it up — the [LLM internals reference card](/learn/llm-foundations/llm-internals-reference-card) is the cheat sheet, not a substitute for knowing it.
  </Step>
  <Step title="The four guided experiments are run and explained">
Not just performed. "The output got weirder" is not an answer; "removing the causal mask lets later tokens leak information backward, so the model can partially solve the task by looking ahead instead of predicting it" is.
  </Step>
</Steps>

<InlineCheck
  question="What separates milestone 5 (the four guided experiments) from simply running them?"
  options={["Explaining what happened by referencing the specific mechanism, not just describing that the output changed", "Running each experiment exactly three times for statistical significance", "Getting the model to a lower loss than in milestone 2"]}
  answer={0}
  explanation="The lesson's own example draws the line explicitly: 'the output got weirder' is not an answer, while an explanation naming the actual mechanism (like causal-mask removal letting information leak backward) is."
/>
...
<Callout kind="production" title="Order matters">
...
1. Point at the exact tensor where 'what comes next' is decided. How does it get computed, step by step?
2. Your loss curve — what does each bend correspond to mechanically, and where is it lying to you?
3. If you doubled the context length, which parameter count grows, which stays flat, and what breaks first at inference?
4. Show a failure mode of your model and explain it in terms of training data or architecture, not 'the model is small'.
5. What would you change to make the model's next-token choice less confident at the same accuracy — and why might you want that?
