...
<InlineCheck
  question="Which part of a transformer block's cost grows quadratically with sequence length, rather than linearly?"
  options={['Attention, because it builds an n × n matrix of pairwise scores', 'The feed-forward layer, because it widens the hidden dimension', 'The projections in and out of attention, because they touch every token']}
  answer={0}
  explanation="Attention scores every pair of positions, which means an n × n matrix — work proportional to n². The feed-forward layer and the in/out projections each touch every token once, which is work proportional to n."
/>
...
<InlineCheck
  question="Going from a 2k to a 128k context multiplies sequence length by 64. By what factor does the attention score matrix grow, and by what factor does the KV cache grow?"
  options={['Both grow by 64x', 'The score matrix grows by 64² = 4,096x; the KV cache grows by 64x', 'The score matrix grows by 64x; the KV cache stays the same size']}
  answer={1}
  explanation="The score matrix is n × n, so a 64x increase in sequence length squares to a 4,096x increase in matrix size. The KV cache stores one key and value vector per token, so its size scales linearly — a 64x increase in tokens is a 64x increase in cache size."
/>
...
<Callout kind="breaks" title="FLOPs and memory are different problems">
...
<Callout kind="breaks" title="The cost is per-layer, per-head, and it compounds">
...
<Callout kind="breaks" title="A bigger context window is not a flat-rate upgrade">
...
The rest of this module is a tour of ways to dodge this cost: sharing keys and values across heads ([From MHA to MQA: Why Grouped-Query Attention Won](/learn/llm-foundations/multi-query-and-grouped-query-attention)), computing attention exactly but without ever materializing the full matrix ([FlashAttention](/learn/llm-foundations/flash-attention-intuition-and-tiling)), restricting which token pairs can interact at all ([Sparse, Sliding-Window, and Linear Attention](/learn/llm-foundations/sparse-sliding-and-linear-attention)), and replacing attention altogether with a fixed-size recurrent state ([Beyond Attention: State-Space Models and Mamba](/learn/llm-foundations/attention-alternatives-ssms-and-mamba)).
