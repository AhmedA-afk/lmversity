...
<InlineCheck
  question="Two MoE models have very different total parameter counts. What does that tell you about their relative inference cost?"
  options={['Nothing reliable by itself — inference cost tracks active parameters per token, not total parameters', 'The model with more total parameters is always slower to run', 'Total parameter count and inference cost always scale together for any model']}
  answer={0}
  explanation="Per-token compute and latency track active parameters — the experts actually selected by the router — not total parameters sitting in memory. Two MoE models can have very different totals and similar inference cost, or the reverse, depending on how sparse the routing is."
/>
...
<InlineCheck
  question="For a small model with a 32,000-token vocabulary, a 512-dimensional hidden size, and an untied unembedding matrix, roughly how do the combined embedding and unembedding parameters compare to a 20-30M-parameter rest of the network?"
  options={['They are negligible in comparison, well under 1M parameters', 'They are comparable to or larger than the rest of the network — roughly 32.8M combined', 'They cannot be computed without knowing the number of transformer layers']}
  answer={1}
  explanation="32,000 × 512 = 16.4M parameters per matrix; with an untied unembedding matrix that is roughly 32.8M combined — comparable to or larger than a 20-30M-parameter rest of the network, even though the lookup itself is cheap."
/>

### Inferring parameter count from a quantized file's size

<Callout kind="breaks" title="File size tracks bytes per parameter, not parameter count">
File size on disk reflects `parameter count × bytes per parameter`, and quantization changes the second term without touching the first. A 7B-parameter model quantized to 4-bit occupies roughly 3.5 GB, not because it has fewer parameters than a 7B model stored at fp16 (which would be roughly 14 GB), but because each parameter is stored in fewer bits. Backing out a parameter count from file size using the wrong assumed bytes-per-parameter gives a wrong answer in either direction.
</Callout>
