...
<Steps>
  ...
  <Step title="Concatenate heads back together">
    ```python
  ...
  <Step title="Apply the output projection">
    ```python
    output = merged @ W_o
...
<InlineCheck
  question="Why does reversing the reshape-then-transpose order when splitting into heads scramble the data?"
  options={[
    'Reshape only reinterprets the existing memory layout without moving data, so transposing first and reshaping second would group the wrong values together',
    'Reshape and transpose always produce the same result regardless of order, so this cannot cause a bug',
    'The order only matters when d_model is odd'
  ]}
  answer={0}
  explanation="Reshape assumes the last axis is laid out as h contiguous chunks of head_dim each. Transposing before reshaping breaks that assumption, because transpose reorders which values sit next to each other before reshape has a chance to group them by head correctly."
/>
...
<InlineCheck
  question="Before splitting into heads, what must be true about d_model and h?"
  options={[
    'd_model must be evenly divisible by h, so each head gets a whole head_dim with nothing left over',
    'd_model must be larger than the sequence length',
    'h must always equal the sequence length'
  ]}
  answer={0}
  explanation="head_dim = d_model // h, and split_heads reshapes assuming d_model splits evenly into h chunks. A head count that doesn't divide the model dimension evenly is a configuration error, which is exactly why the setup code asserts d_model % h == 0 up front."
/>

## Harden it

<Callout kind="production" title="Before shipping this">
...
**Related:** [Multi-Head Attention: Why Many Heads](/learn/llm-foundations/multi-head-attention-why-many-heads), [Scaled Dot-Product Attention in Numpy](/learn/llm-foundations/scaled-dot-product-attention-in-numpy), [Transformer Block Wiring Bugs](/learn/llm-foundations/transformer-block-wiring-bugs), [Assemble One Full Transformer Block](/learn/llm-foundations/assemble-one-full-transformer-block); [Understand multi-head and cross-attention](/learn/deep-learning/sequence-generative/310-multihead-and-cross-attention)
