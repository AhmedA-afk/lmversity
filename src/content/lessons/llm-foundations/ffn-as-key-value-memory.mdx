...
<InlineCheck
  question="Structurally, what is the key difference between attention's key-value lookup and the FFN's key-value lookup, according to this lesson?"
  options={['The FFN has no values, only keys', 'Attention’s keys and values are computed fresh from the current context every forward pass; the FFN’s keys and values are fixed after training, baked into the weights', 'Attention and the FFN are identical in every respect, including how their weights are constrained']}
  answer={1}
  explanation="Both are weighted sums of value vectors gated by a similarity score against a key. The difference that matters is when the keys and values are set: attention's every forward pass, from the current context; the FFN's once, at the end of training."
/>
...
<Compare title="GeLU vs. SwiGLU">
  <Pane title="GeLU" tone="neutral">
    A smooth approximation to a step function, used in GPT-2/GPT-3-era models. It's a straightforward `activation(x @ W1) @ W2` — exactly the two-matrix shape above.
  </Pane>
  <Pane title="SwiGLU" tone="neutral">
    Used in LLaMA and many subsequent open models, adds a second, separate linear projection that gates the first: roughly `(activation(x @ W1) * (x @ W3)) @ W2`, with `W1`, `W2`, and `W3` all learned. The extra gating term lets the network modulate how strongly a detected pattern gets passed through, based on the input itself, rather than passing every detected pattern through with a fixed activation curve.
  </Pane>
</Compare>
...
<Callout kind="breaks" title="A lens, not a complete account">
...
<InlineCheck
  question="Why does the lesson call the key-value framing 'a lens, not a complete mechanistic account'?"
  options={['Because it has been experimentally disproven and no longer describes the FFN accurately', 'Because some of the FFN’s function is genuinely distributed across many neurons jointly, and editing one row of W2 does not reliably edit one fact', 'Because the framing only applies to attention, not to the FFN']}
  answer={1}
  explanation="The framing explains a real, useful chunk of what the FFN does, but not all of it — some function is distributed across many neurons, and facts are often represented redundantly, so single-neuron edits work better as illustrations than as a general editing method."
/>
