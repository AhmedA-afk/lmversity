...
<Callout kind="breaks" title="Recall degrades exactly where you'd expect">
...
<InlineCheck
  question="Why does counting the r's in 'strawberry' require the model to recall rather than perceive the letters?"
  options={["Because the tokenizer strips vowels before the model sees the word", "Because the word is split into multi-character tokens (straw, berry), and the individual characters inside a token are not separately present anywhere in the model's input", "Because counting letters requires a separate arithmetic module the model doesn't have"]}
  answer={1}
  explanation="A transformer's only input is token embeddings. If a word is one multi-character token like straw or berry, the individual characters inside it aren't separately present in the input — whatever the model knows about spelling has to be recalled from the token ID as a learned fact, not read off a directly perceived sequence of letters."
/>
...
<Callout kind="breaks" title="No token-to-token correspondence between the two numbers">
...
<InlineCheck
  question="Why does chunking '47582' into '475' + '82' and '215' into just '215' make addition harder, even though each chunk only has three digits or fewer?"
  options={["Because the chunks aren't aligned by place value across the two numbers — '215' covers hundreds, tens, and units, with no matching boundary in the first number's chunking", "Because three-digit tokens are always mispronounced by the model", "Because addition requires the numbers to have the same number of digits before tokenization happens"]}
  answer={0}
  explanation="Digit-run capping controls how many digits land in one token, but it doesn't anchor chunk boundaries to place value. '215' bundles hundreds, tens, and units into one token with no counterpart chunk boundary in a differently-sized number, so there is no token-to-token correspondence that lines up with place value the way right-aligned handwritten digits do."
/>
...
<Callout kind="production" title="Two mitigations that follow from the mechanism">
