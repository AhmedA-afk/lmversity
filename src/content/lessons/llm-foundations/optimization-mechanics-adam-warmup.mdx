...
<InlineCheck
  question="A parameter has noisy, large-magnitude gradients from step to step. What does Adam's division by sqrt(v) do to its effective step size, relative to a parameter with small, consistent gradients?"
  options={['It shrinks the noisy parameter’s step size automatically, since v is larger for it', 'It has no effect — all parameters get the same step size regardless of gradient history', 'It increases the noisy parameter’s step size to compensate for the noise']}
  answer={0}
  explanation="v is the running average of squared gradients, so a noisy, large-magnitude gradient produces a larger v and a smaller step after dividing by sqrt(v). A parameter with small, consistent gradients gets a relatively larger step."
/>
...
**Batch size in tokens.** Because sequences differ in length and are often packed together (see [Inside the Pretraining Data Pipeline](/learn/llm-foundations/pretraining-data-pipeline)), "batch size = N sequences" doesn't fix how much signal is in the batch.

<Callout kind="production" title="Batch size is measured in tokens, not sequences">
Token count is what actually determines how averaged, and how low-variance, a single gradient estimate is — which is also the unit [scaling laws](/learn/llm-foundations/scaling-laws-what-they-predict) reason about.
</Callout>
...
<Callout kind="breaks" title="What goes wrong if warmup is skipped">
Large early updates hit randomly-initialized weights at the same moment Adam's moment estimates are least reliable, which is a well-understood source of early loss spikes or outright divergence — especially in deeper, pre-norm models sensitive to early instability. Recovering usually means restarting from an earlier checkpoint with warmup restored, which is exactly why virtually every published training recipe includes it despite the added complexity.
</Callout>
...
<InlineCheck
  question="Why is 'ignoring gradient clipping' listed as more dangerous at larger model scale, not less?"
  options={['Because instability tends to show up more, not less, as depth and width increase, so clipping’s safety margin matters more at scale', 'Because larger models never produce anomalous batches, so clipping becomes unnecessary', 'Because gradient clipping only applies to models below a certain parameter count']}
  answer={0}
  explanation="The lesson states instability tends to increase, not decrease, with depth and width, which is exactly why the safety margin gradient clipping provides matters more, not less, as models scale up."
/>
