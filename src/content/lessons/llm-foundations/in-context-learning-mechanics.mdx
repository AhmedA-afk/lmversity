...
<InlineCheck
  question="What does a previous-token head write into each position's residual stream?"
  options={[
    'Information about the token one position back, so position i carries a trace of what sat at position i-1',
    'A full copy of the current token’s embedding, unchanged',
    'The identity of the token at the very start of the sequence, regardless of position'
  ]}
  answer={0}
  explanation="The previous-token head, one layer earlier, writes a trace of the token one position back into each position's residual stream. That trace is exactly what the induction head's query later matches against to find an earlier occurrence."
/>
...
<Callout kind="note" title="Copying, not reasoning">
...
<Callout kind="note" title="Two different claims, easy to conflate">
...
<Callout kind="note" title="Not the whole story of ICL">
...
<InlineCheck
  question="What's the key difference between the induction-head account and the 'ICL is implicit gradient descent' hypothesis?"
  options={[
    'The induction-head account is a concrete, verified circuit; the implicit-gradient-descent hypothesis is a more speculative claim about the net effect of attending over examples',
    'They are two names for exactly the same mechanism, just described at different levels of detail',
    'Induction heads only apply to code generation, while the gradient-descent hypothesis only applies to natural language'
  ]}
  answer={0}
  explanation="Induction heads are a specific, identified attention circuit verified by inspecting attention patterns directly. Whether the net effect of attention over examples resembles an optimizer step is a separate, more contested question, not settled by the induction-head finding."
/>
...
**Related:** [In-Context Learning](/learn/llm-foundations/in-context-learning), [What Different Heads Learn](/learn/llm-foundations/what-different-heads-learn), [Attention Mechanism, Explained](/learn/llm-foundations/attention-mechanism-explained), [Causal Masking](/learn/llm-foundations/causal-masking); [Context Window Mechanics](/learn/llm-foundations/context-window-mechanics)
