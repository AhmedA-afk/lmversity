...
<InlineCheck
  question="Which part of the system is actually 'autoregressive' — the model or the loop around it?"
  options={['The model itself, which has an internal autoregressive layer', 'The loop wrapped around the model, which feeds the model its own previous output as new input', 'Neither — autoregressive is just another name for the forward pass']}
  answer={1}
  explanation="The forward pass alone is a single, stateless function call with no memory of anything. Autoregression is a property of the loop: it takes the model's own output, appends it, and feeds the whole growing sequence back in as the next input."
/>
...
<Callout kind="breaks" title="The model doesn't plan the sentence before generating it">
...
<Callout kind="breaks" title="The naive loop recomputes work it doesn't have to">
...
<Callout kind="breaks" title="A missing stop condition is a real failure, not a hypothetical one">
...
<InlineCheck
  question="Why does the naive version of this loop waste computation as the sequence grows?"
  options={['Because sampling itself gets slower with more tokens', 'Because step t reprocesses attention over all t previous tokens, even though a token’s key and value never change once computed', 'Because the model has to re-tokenize the entire prompt at every step']}
  answer={1}
  explanation="A token's key and value are fixed the moment they're computed, so recomputing them at every later step is pure waste. That's exactly the redundancy the KV cache is built to eliminate."
/>
