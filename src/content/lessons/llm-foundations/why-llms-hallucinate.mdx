...
<InlineCheck
  question="What is missing from the next-token training objective that would let a model output something like 'I don't know'?"
  options={["Nothing is missing — the model chooses not to say it for stylistic reasons", "A class or label for 'I don't know' — ordinary text isn't annotated with which of its claims the next reader will find false", "The model's vocabulary simply doesn't contain the words needed to express uncertainty"]}
  answer={1}
  explanation="Next-token prediction on raw internet text never sees a reject-option label, because ordinary text isn't annotated with which of its claims are false. A classifier gets a reject option only when someone designs a label for it and trains against it directly."
/>
...
<InlineCheck
  question="According to this section, what determines whether a specific fact comes out correct, as opposed to merely fluent?"
  options={["How often that particular fact showed up in training — fluency is reinforced by nearly every sentence, but factual correctness tracks a much rarer, fact-specific signal", "Whether the model was trained with a larger context window", "The temperature setting used at inference time"]}
  answer={0}
  explanation="Fluency density and content density are controlled by different things. Grammar and sentence templates are reinforced by nearly every sentence regardless of topic, but whether a specific fact is correct depends entirely on how often that particular fact showed up in training."
/>
...
<Callout kind="breaks" title="Temperature 0 doesn't remove hallucination">
Greedy decoding removes sampling *variance* — it still walks straight to the argmax token even when the argmax is a fabricated-but-plausible completion. It can make a fabrication more consistent across runs, not less present.
</Callout>

<Callout kind="breaks" title="A confident tone is not evidence">
Confidence in phrasing is a learned writing style reinforced by fluency density everywhere in training; it carries no information about whether the specific fact underneath has real support.
</Callout>

<Callout kind="breaks" title="A bigger or newer model shrinks the sparse regions, it doesn't remove them">
Scale improves coverage of facts, so hallucination gets rarer on well-known topics — but any question landing outside the model's dense training support can still get a fluent, wrong answer, and better prose can make it a more convincing one.
</Callout>
...
**Related:** [A Hallucination Taxonomy and Its Mitigations](/learn/llm-foundations/hallucination-taxonomy-and-mitigations), [Pretraining: Learning From the Whole Internet](/learn/llm-foundations/pretraining-explained), [Alignment Tax, Reward Hacking, and Sycophancy](/learn/llm-foundations/alignment-tax-reward-hacking-sycophancy), [RLHF, Reward Models, and PPO](/learn/llm-foundations/rlhf-reward-models-and-ppo) · [Why LLMs Hallucinate, Derived From the Training Objective](/learn/ai-foundations/why-llms-hallucinate)
