...
<InlineCheck
  question="A single sliding-window attention layer has a window of 4,096 tokens. Can that layer attend to a token 5,000 positions back?"
  options={['No — that information is architecturally unreachable to that layer, though depth can partially compensate since a signal can propagate further after passing through additional layers', 'Yes, sliding-window attention always sees the full sequence, it just weighs distant tokens less', 'No — sliding-window attention never lets any information travel more than one window regardless of depth']}
  answer={0}
  explanation="A single sliding-window layer genuinely cannot see past its fixed window — it's not a soft preference, it's structurally excluded. But stacking layers lets a signal propagate further, since each additional layer can carry information one more window's width."
/>
...
<Compare title="Sliding-window vs. global + local">
  <Pane title="Sliding-window alone" tone="neutral">
    Captures local structure cheaply, but any information farther apart than the window is architecturally unreachable within a single layer.
  </Pane>
  <Pane title="Global + local" tone="neutral">
    Adds a small set of global tokens that every position can route through, giving pure sliding-window attention a low-cost bridge for long-range facts — but only if the right tokens are made global.
  </Pane>
</Compare>
...
<Callout kind="breaks" title="Flatter, less selective attention than softmax">
...
<InlineCheck
  question="Why does linear attention avoid ever forming the full n x n score matrix?"
  options={['Because matrix multiplication is associative, so φ(K)ᵀV can be computed first as a d × d matrix that never scales with sequence length, instead of computing φ(Q)φ(K)ᵀ first', 'Because it discards most of the sequence before attention runs', 'Because it only computes attention for a fixed-size window like sliding-window attention does']}
  answer={0}
  explanation="Linear attention replaces softmax(QKᵀ)V with φ(Q)(φ(K)ᵀV). Because matrix multiplication is associative, φ(K)ᵀV can be computed first, producing a d × d matrix that never depends on sequence length — the n × n matrix is never formed at all."
/>
...
<Callout kind="production" title="Match the pattern to where long-range information actually needs to travel">
...
**Related:** [The Quadratic Attention Bottleneck](/learn/llm-foundations/the-quadratic-attention-bottleneck), [FlashAttention: The Tiling and Online-Softmax Idea](/learn/llm-foundations/flash-attention-intuition-and-tiling), [Beyond Attention: State-Space Models and Mamba](/learn/llm-foundations/attention-alternatives-ssms-and-mamba), [What Actually Limits the Context Window](/learn/llm-foundations/context-window-mechanics-and-limits)
