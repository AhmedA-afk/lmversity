...
<InlineCheck
  question="Why can a model's final prediction be decomposed back into each layer's individual contribution?"
  options={['Because every layer writes an additive term into the stream, so the final value is literally the sum of all those terms', 'Because each layer fully overwrites the stream, so only the last layer’s output matters', 'Because layer norm removes each earlier layer’s contribution before the next layer runs']}
  answer={0}
  explanation="The residual stream is a running sum of additive contributions from every sublayer. Because addition is the only operation touching the stream itself, the final value can be read back as the sum of what each layer added — the basis for techniques like the logit lens."
/>
...
<Callout kind="breaks" title="Two different problems, easy to conflate">
The residual connection (the `+`) solves a gradient problem: it gives gradients a direct path back to earlier layers. Layer norm solves a separate scale problem the residual stream itself creates: without it, nothing stops activation magnitude from drifting or growing as depth increases.
</Callout>
...
<Callout kind="breaks" title="Thinking of the stream as “mostly attention's output”">
It's the *sum* of every sublayer's contribution so far, including the original embedding — by a late layer, the running total may contain far more accumulated history than any single sublayer's most recent addition.
</Callout>

<Callout kind="breaks" title="Confusing layer norm with batch norm">
Layer norm normalizes across one token's features; it has nothing to do with statistics computed across a batch of examples, and doesn't behave differently at inference time the way batch norm can.
</Callout>

<Callout kind="breaks" title="Assuming normalization changes what information is present">
Layer norm rescales magnitude and (optionally) shifts and scales per-feature — it doesn't add, remove, or mix information across tokens or features the way attention or the FFN do; it's stabilization, not computation.
</Callout>
