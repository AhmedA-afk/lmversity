...
<Steps>
  <Step title="The current query is at the second “cat.”">
Its query vector encodes something like "I am the token 'cat'; where else has this exact token appeared before?"
  </Step>
  <Step title="An earlier key matches: the first “cat.”">
That earlier position's key encodes "I am 'cat,' at this position." The query-key dot product between the second "cat" and the first "cat" is high, because token identity is what this head's projections are tuned to compare.
  </Step>
  <Step title="Attention lands there, but the useful information is one position over.">
This is what makes induction heads a genuinely two-step, two-head mechanism in most trained models: a first head (a "previous-token head") first copies information from position `t-1` into position `t`'s representation — so the representation *at* "sat" carries a trace of "the token before me was 'cat.'" The induction head's key is built from that shifted representation, so matching the query against it effectively matches against "the token that followed an earlier 'cat.'"
  ...
  <Step title="The value carried back is “sat.”">
Because the key at that shifted position encodes "cat came before me," and the corresponding value is (or points toward) "sat," the second "cat" ends up attending strongly to "sat" and copying it forward.
  </Step>
  <Step title="The output prediction favors “sat” as what comes next.">
The model has, without any explicit copying instruction in its architecture, implemented "the pattern that followed this token last time will follow it again."
  </Step>
</Steps>
...
<InlineCheck
  question="In the grid above, why does position 4 (the second A) place most of its attention weight on position 2, rather than on position 1 (the earlier A)?"
  options={['Because the induction rule is "A B ... A → B": the head attends to whatever token followed the earlier occurrence of the repeated token, not the earlier occurrence itself', 'Because position 1 has already been used and cannot be attended to again', 'Because attention always favors positions closer to the middle of a sequence']}
  answer={0}
  explanation="An induction head matches the current token against an earlier occurrence of itself, but the value it copies forward comes from the position right after that earlier occurrence — B in this case — not from the earlier A itself."
/>
...
<Callout kind="breaks" title="The clean story above is an idealization">
