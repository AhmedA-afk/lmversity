...
<InlineCheck
  question="Why doesn't memorizing '2 + 2 = 4' let a next-token predictor also get '384 + 91 = ___' right?"
  options={["Because the training corpus never contains any arithmetic besides '2 + 2 = 4'", "Because the corpus contains millions of different addition combinations that can't all be individually memorized, so getting most of them right requires internalizing addition itself", "Because larger numbers use a different tokenizer than small numbers"]}
  answer={1}
  explanation="The training corpus contains '17 + 26 = ___', '384 + 91 = ___', and millions of other combinations that can't all be memorized individually. Getting most of them right requires something closer to internalizing addition, not a lookup table of sums."
/>

## The wrong intuition: "isn't this just memorized phrases?"

<Callout kind="note" title="Word-adjacency statistics run out exactly where structure is needed">
...
<InlineCheck
  question="Why can't a simple bigram or trigram model (counting which words tend to follow which) solve '384 + 91 = ___'?"
  options={["Because bigram models only work on French text", "Because the pattern isn't in the local word sequence — it's in the underlying structure the words describe, which local word-adjacency statistics don't capture", "Because bigram models require more training data than exists on the internet"]}
  answer={1}
  explanation="Naive word-adjacency statistics do fine on short, common phrases like 'hot dog', but they have no way to solve an arithmetic problem they haven't seen before, because the pattern isn't in the local word sequence — it's in the underlying structure the words describe."
/>

## When the analogy breaks

<Callout kind="breaks" title="The game rewards plausible, not true">
