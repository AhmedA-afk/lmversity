...
<InlineCheck
  question="Training loss keeps falling nicely on a small SFT set, but validation loss starts rising. What does this pair of trends indicate?"
  options={['The model is learning the general skill faster than expected', 'The model is likely memorizing the small training set rather than learning the general skill', 'Validation loss is unreliable and should be ignored in favor of training loss']}
  answer={1}
  explanation="A falling training loss paired with rising validation loss is the signature of overfitting — the model is driving down loss on specific memorized examples rather than the underlying skill, exactly the gap this mistake describes."
/>
...
<Callout kind="breaks" title="Catastrophic forgetting of pretrained skills">
...
<Callout kind="breaks" title="Reaching for RLHF or DPO when plain SFT would do">
...
<InlineCheck
  question="A team runs DPO on preference pairs to fix a behavior gap, but the model doesn't improve. According to this lesson, what is the most likely cause?"
  options={['DPO always requires more preference pairs than the team collected', 'Both response options being ranked were already missing the target skill, so there was nothing for the preference signal to push toward', 'The learning rate for DPO was set too low']}
  answer={1}
  explanation="Preference optimization teaches a relative ranking between plausible responses. If neither response in a pair demonstrates the target behavior at all, ranking them teaches nothing — that is an SFT gap, not a preference gap."
/>
