...
<Callout kind="breaks" title="Without the mask, the model can attend to its own answer">
Without the triangular mask from [Causal Masking Mechanics](/learn/llm-foundations/causal-masking-mechanics), every position can attend to every other position, including ones later in the sequence. During training on the next-token objective, the model can partially "cheat" by attending directly to the token it's supposed to predict.
</Callout>
...
<InlineCheck
  question="Why does a missing causal mask make training loss look implausibly good, while inference quality is much worse than that loss would predict?"
  options={['During training the model can attend directly to the token it is supposed to predict, artificially driving loss down; at inference there is no future to peek at, so the shortcut is gone and the model performs at its real, unpracticed level', 'A missing mask only affects the first token in a sequence, so most of the loss curve is unaffected', 'Missing masks make the model faster, which is mistaken for better loss']}
  answer={0}
  explanation="Without the mask, every position can see every other position, including the one it's predicting. That lets training loss fall unrealistically fast, but at inference — where the future genuinely isn't available — the model never learned to do the task honestly."
/>
...
<Callout kind="breaks" title="Deep stacks need the exact form x = x + Sublayer(...)">
The residual connection from [The Residual Stream and Layer Norm](/learn/llm-foundations/residual-stream-and-layer-norm) depends on the specific form `x = x + Sublayer(...)`. Writing `x = Sublayer(x + x)` (doubling the input into the sublayer instead of adding the sublayer's output afterward) or simply `x = Sublayer(x)` (overwriting `x` instead of adding to it) both remove the direct gradient path back to earlier layers.
</Callout>
...
<InlineCheck
  question="A team reports loss spikes partway through training with no single obvious point of failure, and suspects norm placement. What would confirm a norm-placement bug specifically?"
  options={['The raw x tensor added at a residual + has already been passed through a norm somewhere upstream, so the residual path is no longer normalization-free', 'The loss curve goes down smoothly with no spikes at all', 'The attention weight rows sum to exactly 1 at every position']}
  answer={0}
  explanation="A norm-placement bug is about which tensor gets normalized and when, not about a missing operation. If the value added back at a residual + was normalized upstream, the residual path has lost the normalization-free property pre-norm depends on."
/>
