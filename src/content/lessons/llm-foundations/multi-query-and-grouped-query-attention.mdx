...
<Compare title="Full multi-head attention vs multi-query attention">
  <Pane title="Full MHA" tone="neutral">
...
Multi-query attention (MQA) is the extreme opposite: keep all h query heads, but collapse the key and value heads down to just one shared pair. Every query head reads from the same K and V.
  </Pane>
</Compare>

Grouped-query attention (GQA) sits between the two: split the h query heads into g groups, and give each group its own shared K/V pair. Set g = h and you're back to full multi-head attention; set g = 1 and you have MQA. GQA is the tunable dial between "maximum quality, maximum memory" and "minimum memory, some quality cost." The mechanics of that dial, the memory math, and the "uptraining" recipe for retrofitting it onto an existing model are covered in full in [Grouped-Query and Multi-Query Attention](/learn/llm-foundations/grouped-query-attention) — this lesson focuses on why the middle of the spectrum, rather than either end, became the default.
...
<InlineCheck
  question="Why does sharing key/value vectors across a group of query heads hurt quality less than you might expect?"
  options={['Because the query heads are removed entirely, so there is nothing left to lose', 'Because the diversity that matters most for what a head “notices” lives on the query side — each head still computes its own query projection even when the K/V pool is shared', 'Because GQA retrains the entire model from scratch to compensate']}
  answer={1}
  explanation="A head's expressive power comes from what it asks for (its own query projection) as much as from what it retrieves. Queries stay fully diverse even when the K/V pool is shared, since each head specializes in how it weights that shared pool, not in having a private one."
/>
...
<Callout kind="breaks" title="Head count is not KV head count">
...
<Callout kind="breaks" title="GQA cuts memory, not attention FLOPs">
...
<Callout kind="breaks" title="Group boundaries are fixed at training time">
...
<InlineCheck
  question="A model config lists num_attention_heads = 32 and num_key_value_heads = 8. What attention variant is this?"
  options={['Full multi-head attention, since 32 heads is a large number', 'Grouped-query attention with 8 groups', 'Multi-query attention, since key/value heads are shared']}
  answer={1}
  explanation="If num_key_value_heads matches num_attention_heads, it's full MHA; if it's 1, it's MQA. A smaller-but-not-1 value — here 8 — is GQA, with that many shared key/value groups."
/>
