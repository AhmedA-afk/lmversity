...
<Callout kind="breaks" title="No row exists past the trained maximum">
The table has a hard maximum size baked in at training time. There is no row for position 5,000 if `max_sequence_length` was 2,048 — the model has literally never seen, let alone learned anything about, positions beyond that cutoff. Extrapolation isn't just weaker here, it's structurally undefined past the trained maximum, which is precisely the problem RoPE and ALiBi were built to avoid.
</Callout>
...
<InlineCheck
  question="Why can't a learned absolute positional embedding table produce anything useful for a position past its trained maximum?"
  options={['Because there is no row in the table for that position — the model never saw, and never learned anything about, positions beyond the trained maximum', 'Because the embedding values become numerically unstable at long positions', 'Because learned absolute embeddings only work for classification tasks']}
  answer={0}
  explanation="The positional embedding table has a fixed number of rows, one per trained position. Past that maximum there is no learned row at all, so extrapolation is structurally undefined rather than merely weaker."
/>
...
<Compare title="RoPE vs. ALiBi">
  <Pane title="RoPE" tone="neutral">
    Rotates query and key vectors by an angle proportional to position, so attention scores depend only on relative offset. Reasonable generalization somewhat past the trained length, but quality still degrades pushed far enough beyond it.
  </Pane>
  <Pane title="ALiBi" tone="neutral">
    Adds a fixed, position-dependent penalty to attention scores, pushing distant tokens down by construction. Built and evaluated specifically for training short and generalizing long, at the cost of a hard-coded bias toward locality.
  </Pane>
</Compare>

<InlineCheck
  question="ALiBi and RoPE both encode relative rather than absolute position, but at different points in the computation. Where does each act?"
  options={['RoPE rotates the query and key vectors before the dot product; ALiBi instead adds a fixed penalty directly to the attention scores after the dot product, before softmax', 'RoPE and ALiBi both modify the token embedding once, before the first attention layer', 'ALiBi rotates query and key vectors; RoPE adds a bias to the attention scores']}
  answer={0}
  explanation="RoPE rotates Q and K vectors inside the attention computation, before the dot product is taken. ALiBi instead leaves Q and K untouched and adds a fixed, distance-based penalty directly to the resulting attention scores, before softmax."
/>
...
<Callout kind="production" title="Default to RoPE unless the model is already fixed">
If you're fine-tuning or prompting an existing model, this isn't your decision — the scheme is fixed by the architecture you loaded, and it's worth knowing which one so you know what to expect as you approach that model's trained context length. If you're designing a new model: default to RoPE — it's the de facto standard across current open-weight and most production LLMs, composes cleanly with the KV cache, and pairs with well-understood scaling techniques to extend context after pretraining. Consider ALiBi specifically when robust generalization to sequence lengths well beyond training is the priority and you're comfortable with its built-in locality bias. Reach for learned absolute embeddings only when sequence length is small and fixed and simplicity outweighs any need for length flexibility. Treat sinusoidal as a historical baseline and teaching tool rather than a first choice for a new model today.
</Callout>
