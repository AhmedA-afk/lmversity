...
<InlineCheck
  question="In the 4-token mask above, which positions can position 3 legally attend to?"
  options={["Positions 1, 2, and 3, but never 4", "Only position 3, itself", "All four positions, since masking only affects position 4's own row"]}
  answer={0}
  explanation="Row 3 of the mask has -inf only at column 4. Positions 1 through 3 keep a score of 0.0, meaning position 3 may attend to all of them but never to position 4, which comes after it."
/>

## Why negative infinity, specifically

Softmax is `exp(x_i) / sum(exp(x_j))`. `exp(-inf)` is exactly `0.0` in floating point, and `0.0` in both the numerator (for a masked entry) and every masked term in the denominator's sum means those positions contribute nothing to the resulting distribution — not "almost nothing," exactly zero probability.

<Callout kind="production" title="Why -1e9 instead of literal -inf">
In practice, implementations use a very large negative finite number (like `-1e9`) instead of literal infinity to avoid `nan` from `-inf` arithmetic edge cases, but the effect after softmax is the same: those weights round to zero.
</Callout>
...
<InlineCheck
  question="Why does masking after softmax instead of before produce a buggy result?"
  options={["The remaining weights no longer sum to 1 unless you renormalize by hand", "It makes the attention scores negative", "It applies the mask twice per layer instead of once"]}
  answer={0}
  explanation="Softmax's normalization happens before a post-hoc mask would zero out future positions, so zeroing them afterward leaves a set of weights that no longer sums to 1 — a shrunk mixture, not a valid weighted average, unless renormalized."
/>
...
<Callout kind="breaks" title="Common wiring mistakes">
...
**Related:** [Attention as a Soft, Differentiable Lookup](/learn/llm-foundations/attention-as-soft-lookup), [Watching the Mask Change the Softmax](/learn/llm-foundations/watching-the-mask-change-the-softmax), [Next-Token Prediction](/learn/llm-foundations/next-token-prediction), [The Autoregressive Generation Loop](/learn/llm-foundations/the-autoregressive-generation-loop); [Causal Masking: Why LLMs Can't Peek Ahead](/learn/llm-foundations/causal-masking)
