...
<InlineCheck
  question="Why does dividing the raw dot product by sqrt(d_k) specifically — rather than by d_k itself — bring its variance back to 1?"
  options={["Because the dot product's standard deviation grows with sqrt(d_k), so dividing by that same factor cancels it out exactly", "Because d_k is always an even number, and square roots only work cleanly on even numbers", "Because softmax requires its inputs to be scaled by a square root as a matter of definition"]}
  answer={0}
  explanation="Summing d_k independent, zero-mean, unit-variance products gives Var(q · k) = d_k, so std(q · k) = sqrt(d_k). Dividing the dot product by sqrt(d_k) divides its standard deviation by that same factor, bringing the variance back to exactly 1 regardless of d_k."
/>
...
Suppose one query's raw, unscaled scores against three candidate keys come out around `[14, 2, -9]` — magnitudes you'd expect to see occasionally once the standard deviation is 8.

<Compare title="Softmax on the same three scores, unscaled vs scaled by 1/sqrt(64)">
  <Pane title="Unscaled" tone="bad">

Run softmax directly on that:
...
The distribution is, for all practical purposes, one-hot on the first key. That might sound fine — confident predictions! — but it's poison for training. The gradient of softmax output `p_i` with respect to its own logit is `p_i * (1 - p_i)`, which is maximized at `p_i = 0.5` and collapses toward zero as `p_i` approaches 0 or 1. At `p_1 ≈ 0.999992`, that gradient is on the order of `0.999992 * 0.000008 ≈ 0.000008` — vanishingly small. The other two logits are stuck near-zero probability with equally starved gradients. Backpropagation has almost nothing to work with: the model can't learn to adjust *which* key should have won, because the loss surface near this point is nearly flat.
  ...
  <Pane title="Scaled by 1/sqrt(64)" tone="good">
...
<Callout kind="breaks" title="The failure gets worse as d_k grows, not better">
...
<InlineCheck
  question="Why does the fix for this failure have to be a function of d_k, rather than one fixed constant chosen once?"
  options={["Because different tokenizers use different vocabulary sizes", "Because the standard deviation of the raw dot product itself grows with sqrt(d_k), so a fixed constant would undercorrect for large heads and overcorrect for small ones", "Because d_k changes at every layer of the network during training"]}
  answer={1}
  explanation="The raw dot product's spread grows with sqrt(d_k) — a head dimension of 128 gives roughly 11.3, versus about 4 for a dimension of 16. A single fixed constant can't track that growth, but dividing by sqrt(d_k) keeps the logit variance pinned at 1 no matter what head size is chosen."
/>
