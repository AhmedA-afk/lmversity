...
<InlineCheck
  question="In the mixer analogy, what does writing 'you are the 3rd person in this room' onto someone's card correspond to?"
  options={["Positional encoding, injecting order information directly into the content before attention runs", "Causal masking, preventing a token from seeing later tokens", "The softmax step, which turns attention scores into weights"]}
  answer={0}
  explanation="Attention itself only reads what's written on the cards — it has no separate channel for where someone is standing or in what order they arrived. Positional encoding is exactly the act of writing that position onto the card itself, before the mixing starts."
/>
...
<Compare title="Same three tokens, two different orders">
  <Pane title="Order 1: A, B, C" tone="neutral">
...
```
  ...
  <Pane title="Order 2: C, A, B" tone="neutral">
...
<InlineCheck
  question="In the worked example, token A gets the vector [0.8446, 0.5777] whether it's listed first (order1) or second (order2). What does this demonstrate?"
  options={["A bug in the softmax implementation that would be fixed by normalizing differently", "Permutation-equivariance: reordering the input reorders the output the same way, but no token's actual value changes", "That token A is always the most important token regardless of context"]}
  answer={1}
  explanation="Attention is permutation-equivariant: permute the input rows and the output rows come out permuted by the same amount, with every token getting back the identical vector it would have gotten in the original order — just sitting in a different row."
/>
...
<Callout kind="breaks" title="'Equivariant' is not 'invariant'">
The output *reorders* along with the input — it doesn't stay fixed. What's missing is any positional information being used to *change* a token's representation based on its role in the sequence, not the ability to tell inputs apart at all.
</Callout>

<Callout kind="note" title="This isn't specific to language">
Any application of raw self-attention — over image patches, over graph nodes, over a set of database records — has the identical property. Positional (or structural) encoding is what turns a generic set-processing mechanism into a sequence-, grid-, or graph-aware one.
</Callout>

<Callout kind="note" title="Causal masking is a separate concern">
[Causal masking](/learn/llm-foundations/causal-masking) stops a token from attending to future tokens; it does not, on its own, tell a token *where* it sits among the tokens it can see. Both mechanisms are needed together in an autoregressive model.
</Callout>
