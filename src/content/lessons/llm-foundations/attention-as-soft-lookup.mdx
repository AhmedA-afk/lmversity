...
<InlineCheck
  question="In the query, key, value vocabulary, which vector actually appears in a token's output?"
  options={['The value, weighted and blended with every other token’s value', 'The key, since that is what the query is matched against', 'The query, since it determines the weighting']}
  answer={0}
  explanation="The key is only ever used to compute a match score against the query; it never appears in the output. The output is a weighted blend of the value vectors."
/>
...
<InlineCheck
  question="With one-hot value vectors as in the example above, why is the output vector itself readable as the attention weights?"
  options={['Because each one-hot value vector picks out exactly one position, so weighting and summing them reproduces the weight values directly in the output', 'Because one-hot vectors force the softmax to output only 0s and 1s', 'Because the query and key vectors are also one-hot in this example']}
  answer={0}
  explanation="Each V row is 1 in exactly one slot. A weighted sum of vectors that are each 1-in-one-place puts each weight directly into its own output slot, so the output vector and the weight vector are numerically identical in this special case."
/>
...
<Callout kind="breaks" title="Common mix-ups">
