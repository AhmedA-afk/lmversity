...
<Step title="Exponentiate every logit">
...
Exponentiating does two things at once: it makes every value positive (a probability can't be negative), and it exaggerates the *gaps* between scores — a logit that's 2 higher than another becomes roughly `e² ≈ 7.4` times larger after this step, not just 2 units larger.
</Callout>
...
<Step title="Divide each by the sum">
...
<Step title="Rank the candidates">
...
<Callout kind="note" title="Why this step?">
This is the number a sampling strategy actually consumes. Greedy decoding would just take "fly"; a looser [sampling temperature](/learn/llm-foundations/sampling-temperature-top-p) would occasionally still produce "run" or "bark," in roughly the proportions shown.
</Callout>
...
<InlineCheck
  question="After running exponentiate, sum, and divide on a set of logits, what must the resulting values satisfy?"
  options={['They must all be greater than 0.5', 'They must be non-negative and sum to exactly 1', 'They must sum to the same total as the original logits']}
  answer={1}
  explanation="Exponentiating makes every value positive, and dividing by the sum forces the total to 1 — that's the definition of a valid probability distribution, which is exactly what softmax produces."
/>
...
`e^1000` overflows a standard float before you ever get to divide — softmax computed this naively can silently produce `nan` for every probability.

<Callout kind="breaks" title="Naive softmax overflows on large logits">
...
<InlineCheck
  question="Why does subtracting the maximum logit from every logit before exponentiating fix the overflow problem?"
  options={['It changes the resulting probabilities to be more numerically stable but slightly different from the true softmax', 'It is mathematically identical to the original computation (the shift cancels in the division) but keeps every exponent ≤ 0, so exp() never overflows', 'It discards the smallest logits so fewer values need to be exponentiated']}
  answer={1}
  explanation="Subtracting a constant from every logit before exponentiating cancels out exactly when you divide by the sum, so the result is mathematically identical to unshifted softmax. The only thing that changes is that every exponent is now ≤ 0, which never overflows."
/>
