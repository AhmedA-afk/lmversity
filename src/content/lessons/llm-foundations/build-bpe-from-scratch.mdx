...
<Steps>
  <Step title="Represent words as symbol sequences">
...
BPE never looks at "words" directly — it looks at sequences of symbols and merges the ones that co-occur most. Starting every word as individual characters is what makes this "byte-pair" (or here, character-pair) encoding: the smallest possible units, merged upward.

  </Step>
  <Step title="Count adjacent pairs">
...
A pair that appears inside a frequent word should outweigh a pair that appears inside a rare one — that's the whole compression logic of BPE. Weighting by `freq` instead of just counting occurrences per unique word is what makes the merges track real usage in the corpus, not just vocabulary variety.
...
This is a single, mechanical rewrite: wherever `a` is immediately followed by `b`, replace both with one new symbol `a+b`. Applying it everywhere at once, rather than one word at a time, is what lets the same merge propagate consistently across the whole corpus.

  </Step>
  <Step title="Loop until you hit the target vocabulary size">
...
<Callout kind="production" title="num_merges stands in for target vocabulary size">
Each merge adds exactly one new symbol to the vocabulary. `max(pairs, key=pairs.get)` is the entire "learning" in BPE: at every step, look at what's most frequent *right now* (after all previous merges) and fuse it. Nothing is decided in advance; each merge only makes sense in light of the merges before it.
</Callout>

  </Step>
  <Step title="Encode new text with the learned merges">
...
<InlineCheck
  question="What does merge_pair do to every word in the vocabulary?"
  options={["Rewrites every occurrence of symbol a immediately followed by symbol b into one new symbol a+b, across the whole corpus at once", "Deletes any word that does not contain the pair", "Recomputes the frequency of every word from scratch"]}
  answer={0}
  explanation="merge_pair is a mechanical, corpus-wide rewrite: wherever a is immediately followed by b, both get replaced with the single new symbol a+b, applied to every word at once so the merge propagates consistently."
/>
...
<InlineCheck
  question="Why does encoding the word abd succeed instead of failing on the letter d?"
  options={["d never matches any learned merge rule, so it simply survives unmerged as its own raw symbol", "The encoder substitutes the most similar known symbol for d", "The encoder raises an error that the caller must catch"]}
  answer={0}
  explanation="encode only ever applies the fixed list of learned merges. A symbol with no matching rule just falls through every one of them unmerged — the worst case is more, smaller tokens, not a failure."
/>

A production tokenizer applies the same replay logic to real text — try it below on the words this toy corpus was built from:

<Tokenizer text="ab abc bc" />
