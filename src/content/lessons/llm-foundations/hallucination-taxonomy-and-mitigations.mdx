...
[Why LLMs hallucinate](/learn/llm-foundations/why-llms-hallucinate) covers the single root mechanism — no reject option in the training objective. This page classifies the shapes that mechanism produces and goes deeper on what each mitigation does and doesn't reach.
</Callout>
...
<InlineCheck
  question="Why do abstention-trained models still both over-refuse and confidently fabricate?"
  options={[
    'Because rewarding correct refusals requires knowing in advance which questions the model can’t answer reliably, and that’s approximated with imperfect proxies',
    'Because abstention training removes the model’s ability to answer any question with confidence',
    'Because abstention training is only applied after the model has already been deployed'
  ]}
  answer={0}
  explanation="To train a rewarded refusal pathway, you need labels for which questions genuinely can't be answered reliably. That's approximated with proxies like question type or knowledge-cutoff boundaries, and the proxy's imperfections show up as both over-refusal and continued fabrication on questions the proxy doesn't match."
/>

## Mitigation 3: self-verification

Where abstention training changes the model, self-verification changes the *procedure* around a fixed model, spending extra inference compute to check a draft against itself. One concrete form, chain-of-verification (Dhuliawala et al., 2023):

<Steps>
  <Step title="Generate a draft answer">
    The model produces its first-pass response to the question.
  </Step>
  <Step title="Generate independent verification questions">
    A set of questions targeting the draft's specific claims, generated separately from the draft itself.
  </Step>
  <Step title="Answer each verification question separately">
    So the answer isn't contaminated by having already committed to the draft.
  </Step>
  <Step title="Revise the draft wherever a verification answer contradicts it">
    A simpler, cheaper form of the same idea: resample the same claim at nonzero temperature several times and check whether the specific fact holds steady — a claim the model has real support for tends not to move much across resamples, while a fabricated specific, freshly reconstructed from a flatter distribution each time, tends to drift.
  </Step>
</Steps>

<Callout kind="breaks" title="What self-verification doesn't fix">
Self-verification uses the same underlying model and the same mechanism to do the checking, so it inherits the checker's own blind spots — a fact the model is confidently wrong about tends to get confidently re-confirmed rather than caught, because the verification step has no more access to ground truth than the original generation did. It's most useful for catching the *unstable*, sparse-region fabrications described in [why LLMs hallucinate](/learn/llm-foundations/why-llms-hallucinate), and least useful for catching a wrong "fact" the model holds with genuine, consistent (but incorrect) conviction.
</Callout>

<InlineCheck
  question="Why does self-verification tend to fail on a fact the model is confidently and consistently wrong about?"
  options={[
    'The verification step uses the same model and mechanism as the original generation, so it has no more access to ground truth than the answer it’s checking',
    'Self-verification only checks claims that were generated at zero temperature',
    'Self-verification requires a separate, larger model that most teams don’t have access to'
  ]}
  answer={0}
  explanation="Self-verification is the same model checking its own output. A fact it holds with genuine, consistent conviction gets confidently re-confirmed on resampling rather than caught, because the checker has no independent access to ground truth."
/>
