...
<InlineCheck
  question="A model using RoPE is asked to process a sequence longer than any it saw in training. What actually happens?"
  options={['The request fails outright, the same way it would with a learned absolute position table', 'There is no hard table to overflow, but the rotation angles fall outside the range the model learned to interpret well, so quality degrades', 'RoPE has no length dependence at all, so nothing changes']}
  answer={1}
  explanation="RoPE computes angles mathematically rather than looking up a row in a table, so there is no index to run out of — but the frequencies were tuned to a training-length distribution, so far-out positions still degrade rather than error."
/>
...
<InlineCheck
  question="Context length doubles from 4,096 to 8,192 tokens. What happens to KV-cache memory and to attention's pairwise score computations, respectively?"
  options={['Both roughly double', 'Cache memory roughly doubles; attention score pairs roughly quadruple', 'Cache memory roughly quadruples; attention score pairs roughly double']}
  answer={1}
  explanation="Cache memory is linear in token count, so it doubles. Attention score pairs scale with n², so doubling n roughly quadruples them — the two costs of the same context increase move at different rates."
/>
...
<Callout kind="breaks" title="Supported and affordable are different claims">
Treating "the model supports 128K context" as "128K context is cheap to use." A request near the top of a model's advertised window can cost meaningfully more in latency and memory than one at a tenth of that length, for the quadratic reasons above.
</Callout>

<Callout kind="breaks" title="RoPE doesn't mean there's no length limit">
...
<Callout kind="breaks" title="Fitting in the window isn't the same as being used well">
