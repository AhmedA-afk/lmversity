...
<Steps>
  <Step title="Multi-head attention as one callable">
...
Identical to [Implement Multi-Head Attention](/learn/llm-foundations/implement-multi-head-attention), just packaged as one function that takes `x` and returns something the same shape as `x`.
  ...
  <Step title="The feed-forward block">
...
`W1` is `(d_model, 4 * d_model)`, `W2` is `(4 * d_model, d_model)` — the expand-then-contract shape from [The Feed-Forward Block and Its Role](/learn/llm-foundations/the-feed-forward-block-role). Note that `feed_forward` takes and returns `(seq_len, d_model)` too: same shape contract as attention.
  ...
  <Step title="One transformer block, pre-norm">
...
<Callout kind="production" title="Why this order?">
Norm goes *inside* each branch, applied to a copy of `x` before it enters the sublayer — never to `x` itself before the `+`. That's the pre-norm placement from [Why Pre-Norm Won, and What RMSNorm Changes](/learn/llm-foundations/why-pre-norm-won-and-rmsnorm): the raw, unnormalized `x` is what actually gets added back, keeping the residual path clean of any normalization Jacobian. Two independent norms — `ln1_gamma` for attention's branch, `ln2_gamma` for the FFN's branch — because each sublayer needs its own learned rescaling, tuned to what that specific sublayer expects to see.
</Callout>

<InlineCheck
  question="In the pre-norm placement this block uses, what gets added back through the residual connection?"
  options={['The raw, unnormalized x from before the sublayer', 'The normalized copy of x that was fed into the sublayer', 'The output of RMSNorm applied a second time']}
  answer={0}
  explanation="Norm is applied to a copy of x that enters the sublayer, but the raw, unnormalized x is what the sublayer's output gets added back to — that keeps the residual path clean of any normalization Jacobian."
/>

  </Step>
  <Step title="Initialize parameters for one block">
...
<InlineCheck
  question="Why can stacked_blocks feed block 1's output straight into block 2 with no adapter in between?"
  options={['transformer_block returns the same (seq_len, d_model) shape it receives, so its output is already a valid input to the next block', 'The two blocks share the same parameters, so no reshaping is needed', 'The mask automatically resizes the tensor between blocks']}
  answer={0}
  explanation="A transformer block's output shape matches its input shape exactly. That's what lets stacked_blocks call transformer_block again on its own output without any projection or reshaping in between."
/>

## Harden it

<Callout kind="production" title="Harden it">
