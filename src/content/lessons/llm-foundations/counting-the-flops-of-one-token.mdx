...
Everything else in this module gets you a correct mental model of what an LLM computes. This lesson puts real numbers on "how expensive is one token" — useful when you need to reason about cost or latency, skippable if you don't need that yet.
</Callout>
...
<InlineCheck
  question="A 7B-parameter model's linear layers cost roughly 2N FLOPs per generated token. What does N refer to here?"
  options={['The model’s (non-embedding) parameter count', 'The number of layers in the model', 'The sequence length in tokens']}
  answer={0}
  explanation="The 2N rule sums two FLOPs (one multiply, one accumulate) over every parameter in the model's linear layers, so N is the parameter count, not a layer count or a sequence length."
/>
...
<InlineCheck
  question="Why does the attention-FLOPs correction term eventually stop being a small correction to 2N, as context length grows?"
  options={['Because 2N itself grows with sequence length too, so both terms grow at the same rate forever', 'Because the attention term scales with seq_len for each new token, while 2N stays flat, so the attention share grows as context lengthens', 'It never stops being small — the 7.6% figure holds at any context length']}
  answer={1}
  explanation="2N is flat regardless of sequence length, but the per-token attention cost grows linearly with seq_len, so at long enough context lengths the attention term catches up to and then dominates the flat 2N term."
/>
...
<Callout kind="breaks" title="2N is a compute estimate, not a time estimate">
