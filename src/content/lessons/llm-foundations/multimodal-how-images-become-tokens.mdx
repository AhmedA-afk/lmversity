...
<Step title="Patchify and encode">
...
<Step title="Project into the LLM's space">
...
<InlineCheck
  question="Under early fusion, why does adding a high-resolution image reduce the text context budget available for the rest of the prompt?"
  options={['It doesn’t — early fusion keeps images entirely separate from the context window', 'Projected image-patch vectors are inserted directly into the token sequence, so they consume context window budget exactly like text tokens do', 'Early fusion compresses text tokens to make room for image tokens']}
  answer={1}
  explanation="Early fusion splices the projected patch vectors directly into the same token sequence the language model attends over, so each patch counts as a token against the same context window as the prompt's text."
/>
...
<InlineCheck
  question="Doubling an image's linear resolution (e.g. 224×224 to 448×448) at a fixed patch size roughly does what to the number of image tokens?"
  options={['Doubles it, since resolution doubled', 'Quadruples it, since patch count scales with image area, not linear dimension', 'Leaves it unchanged, since patch size is fixed']}
  answer={1}
  explanation="Patch count scales with area, not linear dimension. Doubling both width and height quadruples the number of patches — 256 patches at 224×224 becomes 1,024 at 448×448."
/>
...
<Callout kind="breaks" title="A shared embedding space doesn't mean shared training">
The vision encoder is frequently pretrained on a separate objective (often contrastive image-text matching, not next-token prediction) before ever being connected to the language model — the projection layer is what has to be trained (or fine-tuned) specifically to make the two halves cooperate.
</Callout>

<Callout kind="breaks" title="Understanding and generating images are different capabilities">
Most multimodal chat models are multimodal on the *input* side only — they consume image tokens through the pipeline above but still emit only text tokens via ordinary [next-token prediction](/learn/llm-foundations/next-token-prediction). Producing pixels or audio as output typically requires a separate downstream decoder (a diffusion model, an audio vocoder), not an extension of this same token-prediction loop.
</Callout>

<Callout kind="breaks" title="Patch count, not file size, drives token cost">
A visually simple image at high resolution can cost far more tokens than a visually complex one at lower resolution, because the pipeline counts patches, not information content.
</Callout>
