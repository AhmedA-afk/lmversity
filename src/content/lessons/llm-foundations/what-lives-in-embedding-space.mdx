...
<InlineCheck
  question="What does the king − man + woman ≈ queen result actually demonstrate, according to this lesson?"
  options={['That the relationship between king and man got encoded similarly enough to the relationship between queen and woman that vector arithmetic exposes the parallel', 'That there is a single labeled "royalty" axis deliberately built into the embedding matrix', 'That this kind of arithmetic works reliably on almost any analogy, not just this one']}
  answer={0}
  explanation="Nobody designed a royalty axis. The result shows that a consistent relationship between two pairs of concepts got encoded similarly enough that vector subtraction and addition happen to expose the parallel — on this specific, carefully chosen example."
/>

## The wrong intuition

<Callout kind="breaks" title="Two things about this story don't hold up">
...
**The famous demonstrations mostly used static word embeddings, not a modern LLM's raw token table.**

<Compare title="Static word embeddings vs. a modern LLM's token table">
  <Pane title="Word2vec-style embeddings" tone="neutral">
Trained specifically to be the final semantic representation used directly for downstream tasks, so directions in that space were optimized to be as separable and reusable as possible.
  </Pane>
  <Pane title="A modern LLM's token embedding table" tone="neutral">
Just the *first* layer of a much larger system — its raw rows are an out-of-context starting point, and most of the meaningful, context-sensitive geometry gets built afterward, layer by layer, through [attention](/learn/llm-foundations/attention-mechanism-explained). Looking for clean, isolated "meaning directions" in the raw embedding table alone is looking in the wrong place for a lot of what you'd actually want to find.
  </Pane>
</Compare>
