...
<Step title="Softmax over all four experts">
...
Exponentiate, then normalize by the sum — the same softmax that scores every candidate before any cutoff is applied.

</Step>

<Step title="Keep the top-2 by weight">

Top-2 by weight are expert 0 (0.513) and expert 3 (0.380). Experts 1 and 2 never run — their weights (0.085 and 0.023) are simply thrown away, along with the compute their forward passes would have cost.

</Step>

<Step title="Renormalize the survivors">
...
Rescale just the kept two so they sum to 1, since the full softmax no longer represents the full probability mass once experts 1 and 2 are dropped.

</Step>

<Step title="Combine">

The token's output is `0.574 × Expert0(x) + 0.426 × Expert3(x)`. Every token in a batch runs this same process independently, and typically ends up with a different pair of chosen experts.
...
<InlineCheck
  question="In the worked example, experts 1 and 2 have softmax weights of 0.085 and 0.023. What happens to them?"
  options={['They still run, but their outputs are weighted down to nearly zero in the combination', 'They never run at all — no forward pass, no compute spent, and their weights are discarded before renormalization', 'They are merged into a single averaged expert to save compute']}
  answer={1}
  explanation="Only the top-k experts by weight get a forward pass. Experts outside the top-k — here, experts 1 and 2 — do not run at all; their weights are simply discarded, and the kept weights are renormalized to sum to 1 in their place."
/>
...
<Callout kind="breaks" title="Three ways routing gets misread">
Thinking routing happens per-sequence: it's per-token. Two adjacent tokens in the same sentence can, and often do, route to completely different pairs of experts. Expecting the chosen experts to correspond to human categories — "the code expert," "the math expert" — is also a common misread: real trained routers tend to specialize on shallower statistical patterns, not tidy semantic domains. And it's easy to forget the router itself has to be trained — it's a real parameterized layer (`W_router`) that learns via backpropagation like everything else, and getting it to route *usefully*, rather than collapsing onto a favorite few experts, is its own problem, covered in [MoE Load Balancing and Its Failure Modes](/learn/llm-foundations/moe-load-balancing-and-failure-modes).
</Callout>
