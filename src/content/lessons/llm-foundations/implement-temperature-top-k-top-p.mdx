...
<Steps>
  <Step title="Softmax, as a standalone building block">
  ...
  <Step title="Temperature scaling">
  ...
  <Step title="Top-k: keep a fixed count">
    ```python
  ...
  <Step title="Top-p: keep the smallest sufficient set">
    ```python
  ...
  <Step title="Chain everything into one decode_step">
    ```python
...
<InlineCheck
  question="Why does top-p=0.95 keep 4 tokens (adding garlic) where top-p=0.9 kept only 3?"
  options={[
    'Salt, pepper, and sugar together only reach 0.935 cumulative probability, so clearing a 0.95 threshold requires pulling in garlic as well',
    'Top-p=0.95 always keeps exactly one more token than top-p=0.9, regardless of the distribution',
    'garlic has a higher raw probability than sugar, so it is always included first'
  ]}
  answer={0}
  explanation="Salt, pepper, and sugar sum to 0.935 cumulative probability — enough to clear 0.9 but not 0.95. Reaching a 0.95 threshold requires adding the next-highest token, garlic, which is exactly the adaptive behavior top-p is built for."
/>
...
<Callout kind="breaks" title="top_k_filter breaks silently if k exceeds the vocabulary size">
`np.argsort(probs)[-k:]` with `k` larger than `len(probs)` just returns the whole array — no error, so you'll think you filtered when you didn't.
...
<Callout kind="breaks" title="Ties in top-p can include one token too many or too few">
`np.searchsorted` on a cumulative sum with floating-point ties can land on either side of the boundary depending on tiny rounding differences. For anything production-facing, treat top-p's cutoff as "first index where cumulative mass is `>= p`," computed with an explicit loop or `np.searchsorted(cumulative, p, side='left')`, and test it against a distribution with genuine ties.
</Callout>

<Callout kind="breaks" title="T near 0 divides by a near-zero number">
`logits / 1e-8` produces enormous values that overflow `np.exp` even after max-subtraction, turning into `nan`. Clamp `T` to a small positive floor (like `1e-4`) rather than letting a caller pass `0.0001` and get garbage back with no error.
</Callout>
