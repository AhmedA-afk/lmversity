...
<Compare title="An 8,000-token warehouse vs. a 100,000-token warehouse">
  <Pane title="8,000-token subword vocabulary" tone="neutral">
This warehouse has room for common short words and frequent fragments, but not for less common compounds. A plausible split (illustrative — an actual tokenizer's exact merges will differ, but the *pattern* holds): `Sub` `word` ` token` `ization` ` balance` `s` ` vocab` `ulary` ` size` ` against` ` sequence` ` length` ` control` `.` — **14 tokens**. Long or less-frequent words like "tokenization" and "vocabulary" didn't earn a whole-word slot, so they're built from two or three pieces.
  </Pane>
  <Pane title="100,000-token subword vocabulary" tone="neutral">
This warehouse had room to give whole common words their own dedicated brick, including ones an 8k vocabulary had to split: ` tokenization` ` balances` ` vocabulary` ` size` ` against` ` sequence` ` length` ` control` `.`, plus `Sub` `word` for the less common compound — **about 10 tokens**. Fewer bricks, same sentence.
  </Pane>
</Compare>
...
<InlineCheck
  question="Going from an 8k to a 100k vocabulary shortened the example sentence from 14 tokens to about 10. Why isn't that a clear win?"
  options={['The embedding (and tied unembedding) table grew by more than 12x in exchange for roughly a 30% shorter sequence, and rare tokens in the larger vocabulary get far less training signal', 'A 100k vocabulary always produces exactly the same token count as an 8k vocabulary', 'Shorter sequences always cost more compute than longer ones']}
  answer={0}
  explanation="The parameter cost of the embedding table scales directly with vocabulary size, so a 12x larger table bought only a modest sequence-length reduction. On top of that, the larger vocabulary spreads training signal across many more rare tokens, each seen less often."
/>

## The wrong intuition

<Callout kind="breaks" title="Bigger vocabulary is not a free lunch">
