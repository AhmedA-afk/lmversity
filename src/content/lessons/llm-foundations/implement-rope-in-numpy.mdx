...
<Steps>
  <Step title="Choose one rotation frequency per dimension pair">
    ```python
  ...
  <Step title="Apply the rotation to queries and keys before the dot product">
    ```python
    q = np.array([1., 0., 1., 0.])
    k = np.array([0., 1., 0., 1.])
    ...
    <Callout kind="note" title="Why this step?">
    This is exactly what happens inside every attention head using RoPE: rotate the query by its position, rotate the key by its position, then take the dot product as usual. Nothing else about attention changes — RoPE only touches how `q` and `k` are prepared beforehand.
    </Callout>
  </Step>
</Steps>

<InlineCheck
  question="What does rope_angles control for each dimension pair?"
  options={[
    'How fast that pair rotates per position step — early pairs rotate quickly, later pairs rotate much more slowly',
    'The magnitude of the query and key vectors before rotation',
    'Which tokens are allowed to attend to which other tokens'
  ]}
  answer={0}
  explanation="rope_angles assigns one rotation frequency per dimension pair. Early pairs get a fast-changing angle close to 1 radian per position; later pairs get a much slower one, the same multi-frequency idea sinusoidal encoding uses."
/>
...
<InlineCheck
  question="Why do (m=2, n=5) and (m=7, n=10) give the identical attention score under RoPE, even though the absolute positions are completely different?"
  options={[
    'Both pairs share the same relative offset (-3), and rotating both vectors by an equal shared angle cancels out of the dot product entirely',
    'RoPE ignores position entirely, so any two positions always give the same score',
    'The vectors q and k happen to be orthogonal, which makes every score identical regardless of position'
  ]}
  answer={0}
  explanation="Rotation matrices satisfy R(a)ᵀR(b) = R(b - a), so shifting both positions by the same amount is itself just another shared rotation that cancels out of the dot product. Only the relative offset — the same -3 in both cases — survives into the score."
/>
