...
<Steps>
  <Step title="Predict from 3 tokens">
    ```python
    ...
    The model doesn't get to see "time" as a hint — it has to produce it from just `["Once", "upon", "a"]`, which is exactly the kind of high-probability continuation the training data reinforced thousands of times over.
    </Callout>
  </Step>
  <Step title="Predict from 4 tokens">
    ```python
    ...
    <Callout kind="note" title="Why this step?">
    Notice the input grew by exactly one token, but the forward pass reran attention over *all four* — including the three it already processed a moment ago. Nothing here reuses step 1's work yet.
    </Callout>
  </Step>
  <Step title="Predict from 5 tokens">
    ```python
    ...
    <Callout kind="note" title="Why this step?">
    Same pattern again: 5 tokens in, full attention recomputed over all 5, one new word out. Three steps in, the model has now recomputed "Once," "upon," and "a" three separate times each.
    </Callout>
  </Step>
</Steps>
...
<InlineCheck
  question="At step 2 of the loop above, how many tokens does the forward pass process?"
  options={[
    'All 4 tokens in the sequence so far, including the 3 already processed at step 1',
    'Only the 1 new token that step 1 produced',
    'All 5 tokens, including the one step 2 is about to produce'
  ]}
  answer={0}
  explanation="The naive loop reruns model.forward over the entire prefix every time. At step 2 the sequence has grown to 4 tokens, and all 4 — including the 3 already processed at step 1 — go through the forward pass again."
/>

## Where it breaks (+ fix)

<Callout kind="breaks" title="Recomputing the whole prefix every step is quadratic">
...
<InlineCheck
  question="Why does the KV cache collapse the naive n² recomputation down to roughly n?"
  options={[
    'Because a token’s key and value vectors depend only on tokens at or before it, so they can be computed once and reused instead of recomputed at every later step',
    'Because the KV cache lowers the temperature, which shrinks the candidate set the model has to score',
    'Because caching skips the softmax step entirely for tokens that have already been generated'
  ]}
  answer={0}
  explanation="Causal masking guarantees a token's K and V never depend on tokens that come after it. That's exactly what makes it safe to compute them once, cache them, and reuse them at every later step instead of recomputing the whole prefix."
/>
