...
<Step title="Same unbalanced examples, negative last">
...
<Callout kind="note">
Same 3:1 label ratio as Step 3, but the last demonstration before the test input is now positive instead of negative — reordered specifically to test **recency bias**, the companion finding in the same research: the demonstration closest to the query tends to exert outsized pull, on top of whatever the overall label balance contributes. Two prompts with an *identical* label count can still push toward different outputs on the same ambiguous input, purely from which example sits last.
</Callout>
...
<InlineCheck
  question="Step 3 uses three positive examples and one negative example. According to majority-label bias, why does this pull the output toward positive on an ambiguous input?"
  options={['Because the model reasons explicitly about the 3:1 label frequency before answering', 'Because the demonstrations shift the model’s effective output distribution toward the pattern it saw more often, with no explicit frequency reasoning involved', 'Because three examples is technically too few to count as few-shot, so the model defaults to its zero-shot prior']}
  answer={1}
  explanation="Majority-label bias is a pattern-matching effect, not explicit reasoning about frequency — the demonstrations shift the model's effective output distribution toward the label they showed more often, and this matters most on inputs that don't strongly favor one label on their own."
/>

<InlineCheck
  question="Steps 3 and 4 use the identical 3:1 label ratio. What differs between them, and what finding does that difference test?"
  options={['Nothing differs; the two prompts are identical', 'The order of the examples differs — the negative example is last in Step 3 and not last in Step 4 — testing recency bias', 'Step 4 adds a fifth example, testing whether more examples fix the imbalance']}
  answer={1}
  explanation="Both steps keep the same three-positive-one-negative ratio; only the position of the negative example changes. That isolates recency bias — the pull of whichever example sits closest to the query — from the label-balance effect tested in Step 3."
/>
