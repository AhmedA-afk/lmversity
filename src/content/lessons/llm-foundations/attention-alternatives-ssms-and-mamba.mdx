...
import SsmVsKvCache from '../../../components/diagrams/SsmVsKvCache.astro';
...
<Callout kind="breaks" title="The cost problem this module exists to address">
`O(n²)` compute described in [The Quadratic Attention Bottleneck](/learn/llm-foundations/the-quadratic-attention-bottleneck), and a KV cache that grows linearly forever and never shrinks back down.
</Callout>
...
<InlineCheck
  question="Why does S4's fixed-size state struggle with needle-in-a-haystack recall?"
  options={['The state is a compressed summary of everything seen so far, and compression is lossy by nature', 'S4 has no memory of the past at all', 'S4 only processes the most recent few tokens']}
  answer={0}
  explanation="S4's hidden state is fixed-size precisely because it compresses the whole history into it. Compression is lossy, so one exact distant detail can get lost the way it never would in attention's explicit per-token keys and values."
/>
...
<InlineCheck
  question="What does Mamba's selectivity add on top of S4?"
  options={['The A, B, and C matrices depend on the current input token, so the model decides per token what to keep or forget', 'A KV cache that grows exactly like the one in attention', 'A second, larger fixed-size state that never changes']}
  answer={0}
  explanation="S4's dynamics are input-independent for the whole sequence. Mamba makes A, B, and C depend on the current token, so what the fixed-size state retains or discards is decided content by content — this is what made SSMs competitive with attention on language."
/>
