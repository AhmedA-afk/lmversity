...
Multi-head attention doesn't run attention once over the full embedding width. Instead, it splits the model dimension `d_model` into `h` equal slices, runs a complete, independent attention computation in each slice, and then puts the results back together:

<Steps>

<Step title="Project to Q, K, V">

Project the input to Q, K, V, each `(seq_len, d_model)`.

</Step>

<Step title="Split into heads">

Reshape each into `h` heads of size `(seq_len, d_model / h)`.

</Step>

<Step title="Attend independently per head">

Run scaled dot-product attention independently, per head — its own Q, K, V projections, its own softmax.
...
<Compare title="One head vs many heads">
  <Pane title="A single head" tone="bad">
A single attention operation is a single softmax over a single set of query-key comparisons — one answer, per token, to "what matters here." A sentence usually needs several different answers to that question simultaneously: which word does this pronoun refer to, which word does this verb agree with, which earlier token opened the bracket this one is closing. A lone head has to compromise, blending all of those signals into one weighted average that partially satisfies each and fully satisfies none.
  ...
  <Pane title="Many heads" tone="good">
...
[What Different Attention Heads Learn](/learn/llm-foundations/what-different-heads-learn) tours what real heads in trained models actually end up doing.
...
<InlineCheck
  question="Why is running h narrower attention heads roughly the same total compute as one attention operation over the full width, rather than h times the cost?"
  options={['Because most heads are skipped at inference time to save compute', 'Because each head does proportionally less work over a proportionally smaller dimension, and the heads run in parallel', 'Because narrower heads use a cheaper, approximate softmax']}
  answer={1}
  explanation="Splitting d_model into h slices means each head's attention runs over a dimension that is h times smaller, so the total work across all heads stays close to the cost of one full-width attention operation, and it runs in parallel on the same hardware."
/>
...
<Callout kind="breaks" title="More heads isn't strictly better">
...
<Callout kind="breaks" title="Don't skip the output projection">
...
<Callout kind="breaks" title="Reshaping the heads dimension incorrectly">
...
<InlineCheck
  question="What does dropping the final W_O projection after concatenating heads cost the model?"
  options={['Nothing — concatenation alone already lets heads exchange information', 'The ability to mix information across heads before the next sublayer, since concatenation alone only glues independent chunks side by side', 'The model would fail to train entirely']}
  answer={1}
  explanation="Concatenating heads produces h independent chunks sitting side by side. The learned W_O projection is what lets information from different heads combine and interact — without it, heads never talk to each other downstream."
/>
