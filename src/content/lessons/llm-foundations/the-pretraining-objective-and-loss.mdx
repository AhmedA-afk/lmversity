...
<InlineCheck
  question="What makes it possible to compute a training loss at every position of a document in a single parallel forward pass, instead of one token at a time?"
  options={['Teacher forcing — feeding the model the actual preceding tokens rather than its own guesses', 'Running the model once per token, in sequence, and summing the results afterward', 'Only computing loss at the final position of the document']}
  answer={0}
  explanation="Teacher forcing feeds the ground-truth prefix rather than the model's own guesses, so every position's prediction can be computed independently within the same forward pass — that's what turns a 2,048-token document into up to 2,048 loss terms per pass."
/>
...
<InlineCheck
  question="What is the tradeoff teacher forcing makes during training?"
  options={['It is slower than letting the model generate its own tokens during training', 'It sidesteps compounding errors during training, but the model never practices recovering from its own mistakes, since it always sees the true prefix', 'It requires a differently-shaped loss function than inference uses']}
  answer={1}
  explanation="Teacher forcing always supplies the ground-truth prefix, which avoids one wrong guess corrupting every later position during training. The cost is that the model never practices, during training, the thing it must do at inference: recover from its own mistakes."
/>
...
<Callout kind="breaks" title="A falling loss is not the same as understanding">
A falling loss number is not the same as "understanding" or "correctness." It only means the model's predicted distribution is putting more mass on whatever token actually came next — which can be gamed by memorizing near-duplicate passages in the corpus.
</Callout>

<Callout kind="breaks" title="Raw loss values aren't comparable across tokenizers">
You can't compare raw loss values across models with different tokenizers or vocabulary sizes. A larger vocabulary changes the baseline difficulty of the prediction problem, so match tokenizers (or use a normalized metric) before comparing.
</Callout>

<Callout kind="breaks" title="Low pretraining loss says nothing about being a good assistant">
Low pretraining loss says nothing about whether the resulting model will be a good assistant. That behavior comes from what happens *after* pretraining, not from squeezing this one number lower — see [From Base Model to Assistant](/learn/llm-foundations/from-base-model-to-assistant-pipeline).
</Callout>
